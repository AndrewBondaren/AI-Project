...
| Вопрос | Статус |
|---|---|
| Алгоритм сетки улиц — простая сетка или органичнее (Voronoi кварталы)? Референс: Parish & Müller (2001), tmwhere | открыт |
...
| Механика дорог внутри района и между районами | закрыт — см. tz_structure_connections.md §5 |
| `adjacent_terrain` — проверка связанности (река должна куда-то выходить) | не описано |
| **Footprint города — форма** | **TODO: v1 = квадрат; v2 = псевдо-историчная форма** |
...
**Связь с terrain:** требует terrain v2 (elevation map) — **откладывается до terrain v2**.
