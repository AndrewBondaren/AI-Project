...
| `system_location_mood` | `LocationMoodKey \| None` | `NamedLocation` | `prosperous`, `declining`, … N+1. Omit/`null` → нет mood. [POJO-C-2](./tz_pojo_city_typing.md) | ✅ |
...
|---|---|---|---|
| `system_name` | `DistrictTemplateKey` | required | Уникальный ключ чертежа: `"port_district"`, `"merchant_quarter"`. Не ткань `district_type`. [POJO-C-4](./tz_pojo_city_typing.md) |
...
| `deck` | int | optional | **Ярус** района. Omit → `0` (поверхность). Не этаж здания, не `economic_tier`, не climate hive, **не** разведение двух поселений ([`tz_locations.md`](./tz_locations.md) **LOC-T-3**). Участки копируют значение при packing (`AreaSlot.deck`); не независимая настройка участка. Канон omit. Коллизия xy×z — assembler §7.1. |
...
| 2026-09-19 | **LOC-T-3:** `deck` района не разводит два поселения. SoT [`tz_locations.md`](./tz_locations.md) AABB + запас. |
...
| 2026-09-07 | **POJO-C-2 resolved:** скелет / NL `system_location_mood` — `LocationMoodKey`. |
