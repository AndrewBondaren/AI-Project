...
Примеры (ratio=0.75, max=5, default_passage_height=2):
  z=2  → max(1, 2) = 2    z=3  → max(2, 2) = 2    z=4  → max(3, 2) = 3
  z=6  → max(4, 2) = 4    z=7  → max(5, 2) = 5    z=10 → max(5, 2) = 5 (cap)
  z=20 → max(5, 2) = 5    (cap)
```
...
### 3.9.1 Трёхмерная модель ячеек лестницы

Каждый тип лестницы имеет свою 3D-геометрию — способ раскладки ячеек по z-слоям между `z_lo` (нижний этаж) и `z_hi` (верхний этаж).

**Общие обозначения:**
```
z_lo = min(fr_level.z, to_level.z)     -- base z нижнего этажа
z_hi = max(fr_level.z, to_level.z)     -- base z верхнего этажа
z_height = z_hi - z_lo                 -- высота пролёта в z-юнитах
(ax, ay) = to_anchor                   -- XY-якорь (авторитетная позиция лестницы)
N = stair_length = max(2, ceil(z_height * 1.3))   -- только для straight
```

---

**`straight` (1×N) — ступенчатая раскладка:**

Каждая ступень `i` физически поднята на `i` z-юнитов — ячейки раскладываются по диагонали в пространстве:
...
| Ступень | XY | Z |
|---|---|---|
| 0 (вход с нижнего этажа) | (ax, ay) | z_lo |
| 1 | (ax, ay+1) | z_lo+1 |
| ... | ... | ... |
| N-1 (выход на верхний этаж) | (ax, ay+N-1) | z_hi |

Каждый промежуточный z-слой содержит ровно одну ячейку лестницы.  
Модель корректна только если `N ≈ z_height * 1.3` — для других соотношений требуется интерполяция (v2).

---

**`standard` / `spiral_standard` / `spiral_small` — колонна:**

Лестница занимает фиксированный XY-footprint на **каждом** z-слое от `z_lo` до `z_hi + z_height_upper`:

```
fp = _stair_footprint(stair_type, (ax, ay), z_height, rng)  -- 2×2 или 1×2

z_all_lo = z_lo
z_all_hi = z_hi + z_height(upper_level)   -- до потолка верхнего этажа
...
Спираль физически занимает весь объём колонны — промежуточные z-слои корректны.

---

**`ladder` (1×1) — тонкая колонна:**

То же что и колонна, footprint = 1 ячейка:
```
for z in range(z_lo, z_hi + z_height(upper_level)):
    cell(ax, ay, z)
```

---

**`trapdoor` (1×1) — люк в полу:**

Асимметричная модель: люк существует только на уровне `fr_room`; шахта подвала — на всех z `to_room`:

```
-- Люк: одна ячейка в полу fr_room
cell(fr_anchor.x, fr_anchor.y, fr_level.z)

-- Шахта: одна ячейка на всех z to_room (от основания до потолка)
for z in range(to_level.z, to_level.z + to_level.z_height):
    cell(to_anchor.x, to_anchor.y, z)
```
...
```
LocationPassage.from_x/from_y  = fr_anchor   -- точка входа с нижнего (fr) этажа
LocationPassage.to_x/to_y      = to_anchor   -- точка входа с верхнего (to) этажа
```

`LocationPassage` — единственный авторитетный источник координат точки входа. Ячейка входа (`fr_anchor` на `fr_level.z`) не отличается типом от остальных ячеек лестницы — frontend/движок читают координаты из passage-записи.

---

**Выравнивание якорей (fr_anchor vs to_anchor):**

Для колоннообразных типов (`standard`, `spiral_*`, `ladder`) — `fr_anchor` и `to_anchor` должны совпадать по XY, так как физическая колонна не может находиться в двух разных местах. Генератор вычисляет `to_anchor` и использует его же для `fr_anchor`.

Для `straight` — `fr_anchor` и `to_anchor` тоже совпадают по XY: оба указывают на начало марша `(ax, ay)`. Разница между нижним и верхним этажом — в z-слое клетки `i=0` и клетки `i=N-1`, а не в XY-якоре.

Для `trapdoor` — якоря могут не совпадать (люк и шахта в разных позициях допустимы архитектурно).

> **Решено:** `fr_anchor = to_anchor` для всех колонн (standard, spiral_*, ladder) и straight принудительно в генераторе. `trapdoor` — исключение: люк и шахта могут иметь разные XY.

**Headroom-валидация:**

После построения всех маршей генератор проверяет что над каждой ячейкой лестницы (`staircase`, `stair_anchor`, `stair_floor`) есть минимальный просвет:

```
clearance = world.default_passage_height
для каждой stair-ячейки (x, y, z):
    для dz в range(1, clearance + 1):
...
| Поле | Тип | Обязательность | Описание |
|---|---|---|---|
| `opening_type` | string | required | ref → `StructureElement`. Тип проёма: `window`, `arrow_slit`, `porthole`, `vent`, `hatch`, etc. |
...
**Правило 2 — сегментное симметричное размещение.**

Двери разделяют стену на независимые сегменты. Каждый сегмент обрабатывается отдельно.

```
wall_cells = exterior_wall_cells
    - corner_cells (±2 ячейки с каждого конца стены)
    - door_cells   (±1 ячейка от каждой двери)

-- двери создают разрывы → стена разбивается на сегменты:
segments = split_by_gaps(wall_cells)

-- count распределяется пропорционально длине сегментов:
seg_count[i] = round(count * len(segments[i]) / total_len)

-- в каждом сегменте — симметричное размещение:
spacing = 1
total_width = seg_count * size + (seg_count - 1) * spacing
start_offset = floor((len(segment) - total_width) / 2)
positions = [start_offset + i * (size + spacing) for i in 0..seg_count-1]
```

Если сегмент слишком короткий → log WARNING, пропустить этот сегмент.

**Стратегии распределения (`WallDistributor`):**

| `opening_type` | Класс | Поведение |
|---|---|---|
| `window`, `porthole`, `vent` | `SegmentedDistributor` | per-сегмент, симметрично, `spacing=1` |
| `arrow_slit` | `SolidRunDistributor` | один сплошной блок на длиннейшем сегменте, `spacing=0` |

**Правило 3 — Z-позиция по высоте комнаты (`ZAdjuster`).**

Z вычисляется от `room.z_height` (не от `level.z_height`). Логика зависит от `opening_type`:

```
MiddleCellZAdjuster (window, arrow_slit, porthole, vent):
    z_offset = min((room.z_height + 1) // 2, room.z_height - 1)
    abs_z    = level.z + z_offset

    Нечётная высота: ceil(z_height / 2)  → z_height=5 → z_offset=3
    Чётная высота:   z_height / 2        → z_height=4 → z_offset=2

ShaftZAdjuster (автоматические окна шахт):
    Входные данные: fr_anchor (z_lo), to_anchor (z_top)
    Распределяет Z-позиции равномерно по [fr_anchor, to_anchor]
    Количество выводится автоматически из высоты шахты
    Используется только когда стена шахты является внешней стеной здания

TopWallZAdjuster (открытая стена / парапет) — v2:
    z_offset = room.z_height - 1 или room.z_height - 2
    Активируется когда стена имеет свойство open_top (производное от механики переходов)
```

`ZAdjuster.resolve(z_base, z_height) → list[int]` — возвращает список абсолютных Z.  
`WallDistributor.place()` вызывается для каждого Z из этого списка независимо.
...
"wall_openings": [
  { "opening_type": "window", "frame_material": "wood", "glass_material": "glass",
    "wall": "perimeter", "count_range": [2, 4], "distribution": "evenly", "size": 1 }
]

// Замковая бойница — каменная, без стекла
"wall_openings": [
  { "opening_type": "arrow_slit", "frame_material": "stone", "glass_material": null,
    "wall": "all", "count_range": [2, 6], "distribution": "evenly", "size": 1 }
]

// Каюта корабля — железный иллюминатор, толстое стекло (не бьётся)
"wall_openings": [
  { "opening_type": "porthole", "frame_material": "iron", "glass_material": "thick_glass",
    "wall": "east", "count": 1, "distribution": "centered", "size": 1 }
]

// Современная квартира — панорамное окно на юг
"wall_openings": [
  { "opening_type": "window", "frame_material": "aluminum", "glass_material": "tempered_glass",
    "wall": "south", "count": 1, "distribution": "centered", "size": 4 }
]
```

`opening_type` — любой элемент из `StructureElement` типа opening. Новые типы добавляются в enum и `_WALL_OPENING_TYPES` без изменения остального генератора.

---

**Открытые вопросы 3.10:**

| # | Вопрос | Статус |
|---|---|---|
| OQ-1 | Поле называется `terrain_type` в ТЗ, но `opening_type` в коде. Привести к единому. | CLOSED: `opening_type` |
| OQ-2 | `distribution: "centered"` — есть в ТЗ-примерах (porthole, panoramic), не реализовано. Семантика: один блок по центру стены без разбивки на сегменты. | CLOSED: CenteredDistributor |
| OQ-3 | `glass_material` — ТЗ специфицирует поле, `_opening_cell` его не принимает. v2. | CLOSED: резолв из economic_tier через resolve_material; MapCell.glass_material добавлен |
| OQ-4 | Interior conflict: ТЗ говорит "размещает комната с меньшим `room_id` лексикографически"; код: last-write-wins. | CLOSED: interior_claimed + lex-сортировка; двойные стены делятся на отрезки по комнатам |
| OQ-5 | `count_range` resolution: ТЗ говорит "берётся максимальный count который вписывается симметрично"; код делает `rng.randint(cr[0], cr[1])`. Уточнить семантику. | CLOSED: target=max, min=min; distributor обрезает до вмещающегося; placed < min → skip |
| OQ-6 | Двери как разделители стены на сегменты — реализовано, но Правило 2 описывает flat-список без сегментов. Обновить Правило 2. | CLOSED: Правило 2 переписано |
| OQ-7 | Arrow slit: реализован SolidRunDistributor (сплошной блок); ТЗ-пример показывает `distribution: "evenly"`. Обновить пример и описать поведение arrow_slit отдельно. | CLOSED: таблица WallDistributor в Правиле 2 |
| OQ-8 | `system_facing` у проёмов: не упомянут в ТЗ. Нужно ли записывать направление (для рендеринга, для геймплея бойницы)? | CLOSED: _get_wall_cells возвращает facings dict; _opening_cell пишет system_facing на ячейку |
| OQ-9 | Окно напротив внутренней стены: кейс когда exterior-стена комнаты упирается в стену соседней комнаты без зазора. Что делать — запрещать, разрешать, или это уже обрабатывается геометрией? | CLOSED: handled by geometry (all_fp) + interior_claimed; двойные стены не попадают в exterior отрезки |
| OQ-10 | `window_z` per-room vs per-level: вычислять от `room.z_height` а не `level.z_height`. | CLOSED: MiddleCellZAdjuster в Правиле 3 |
| OQ-11 | Shaft windows (автоматические): если стена шахты является внешней — автоматически размещать окна. Одно или несколько по высоте? | CLOSED: ShaftZAdjuster, равномерно по [fr_anchor, to_anchor] |
| OQ-12 | `glass_material` explicit override: ТЗ разрешает задать в шаблоне явно (`"glass_material": "stained_glass"`), код игнорирует и всегда резолвит из тира. Логика: шаблон → тир. | OPEN |
| OQ-13 | `TopWallZAdjuster` — для открытых верхних стен (бойницы на зубцах замка). Зависит от семантики is_open_top на комнате (производная от transition-механики). | OPEN |
| OQ-14 | `display_facing` на opening-ячейках: `system_facing` пишется, локализованный `display_facing` — нет. v2. | OPEN v2 |
| OQ-15 | `cell_states` (открытое/закрытое/сломанное окно) — не реализовано. v2. | OPEN v2 |
| OQ-16 | Arc-length distribution для круглых комнат — не реализовано. v2. | OPEN v2 |
...
| `validate_template` — когда вызывается, кем, что возвращает | Открыт: вызов при импорте через UI vs при генерации; возвращать список ошибок или бросать исключение; нужна ли partial validation (только структура без проверки реестров мира) |
