...
### 3.10 Алгоритм размещения wall_openings

Размещение окон — полностью алгоритмическое. Шаблон не задаёт количество, позиции или распределение. Обработка ведётся **по каждой внешней стороне каждой комнаты отдельно**.

---

**Правило 1 — только наружу.**  
Exterior-ячейка: сосед по направлению `(x+dx, y+dy)` не принадлежит ни одному room-footprint'у (`all_fp`). Только такие ячейки участвуют в размещении.

---

**Правило 2 — сегменты и позиции.**

Для каждой стороны (north / south / east / west):

```
1. exterior_cells = [c for c in side_cells if neighbor(c, dir) ∉ all_fp], sorted along axis

2. Адаптивный срез углов:
   n = len(exterior_cells)
   if n ≤ 2  → пропустить сторону
   cut = clamp(n − 3, 0, 2)
   available = exterior_cells[cut : n − cut]

3. Исключить door_cells ±1 → создают разрывы в available

4. segments = split_by_gaps(available)
   (края сегментов не обрезаются)

5. Для каждого сегмента:
   count = max(1, len(seg) // 3)
   pos[i] = floor((i + 0.5) × len(seg) / count)   i ∈ [0, count)
```
...
**Правило 3 — Z-позиция (`ZAdjuster`).**

Z вычисляется от `room.z_height`:
...
ShaftZAdjuster (автоматические окна внешних стен шахты):
    Распределяет Z равномерно по [fr_anchor_z, to_anchor_z]
    Количество = max(1, shaft_height // floor_z_height)

TopWallZAdjuster — v2 (открытые верхние стены / парапет)
```

---

**Состояния проёма (`cell_states`) — v2:**

| Состояние | Условие |
|---|---|
| `closed` | default |
| `open` | рама позволяет открытие |
| `broken` | `glass_material.breakable = true` + урон |
| `destroyed` | рама разрушена |
...
| # | Вопрос | Статус |
|---|---|---|
| OQ-1 | `terrain_type` vs `opening_type` | CLOSED: `opening_type` |
| OQ-3 | `glass_material` резолв | CLOSED: из economic_tier через resolve_material |
| OQ-4 | Interior conflict | CLOSED: lex-сортировка по room_id |
| OQ-6 | Двери как разделители сегментов | CLOSED: Правило 2 переписано |
| OQ-8 | `system_facing` на ячейке | CLOSED: пишется при создании opening-ячейки |
| OQ-9 | Двойная стена (exterior упирается в соседнюю комнату) | CLOSED: exterior_cells фильтруются по all_fp |
| OQ-10 | `window_z` от `room.z_height`, не `level.z_height` | CLOSED: MiddleCellZAdjuster |
| OQ-11 | Shaft windows автоматические | CLOSED: ShaftZAdjuster |
| OQ-13 | `TopWallZAdjuster` (зубцы замка) | OPEN v2 |
| OQ-14 | `display_facing` на opening-ячейках | OPEN v2 |
| OQ-15 | `cell_states` реализация | OPEN v2 |
| OQ-16 | Arc-length для круглых комнат | OPEN v2 |
...
| Условие | Результат |
|---|---|
| `level.z < 0` (подземный уровень) | нет проёмов |
| нет exterior-стен у комнаты | нет проёмов |
| `room.is_shaft` | пропускается |

**Правила по `room_type`:** OPEN — OQ-17.

**Интеграция в pipeline (`_place_wall_openings`):**
```
1. all_fp = union всех room footprint'ов (один раз на здание)
2. Для каждой комнаты: resolve → определить opening_type и параметры  ← OQ-17
3. Для каждой стороны каждой комнаты: Правило 2 → разместить ячейки
4. Правило 3 → Z-позиция
```

Файл резолвера: `passages/wallOpeningResolver.py`

**Открытые вопросы 3.11:**

| # | Вопрос | Статус |
|---|---|---|
| OQ-17 | Правила по `room_type`: какие типы получают проёмы, какой `opening_type` | OPEN |
| OQ-18 | Влияет ли `structure_type` на `opening_type` (dungeon → arrow_slit?) | OPEN |
| OQ-19 | Источник материала рамы: только `economic_tier` или `room_type` тоже | OPEN |
| OQ-20 | Глобальный override `no_windows: true` на уровне шаблона | OPEN |
...
## 14. Механика строительства и ремонта

Применима ко всем постройкам (`NamedLocation` типа `building`, `room` и др.) и дорогам (`ConnectionEdge`).  
Описание для дорог: [tz_structure_connections.md](tz_structure_connections.md), раздел 3.2.

### 14.1 Флаги состояния

| Поле | Тип | Описание |
|---|---|---|
| `under_construction` | bool | Постройка строится (новая); недоступна до завершения |
| `under_repair` | bool | Постройка на ремонте (существующая); может ограничивать доступ |

Оба флага хранятся на `NamedLocation`.

### 14.2 Поведение в engine

- `under_construction = true` → постройка не генерирует интерьер до завершения; LLM видит строительные леса
- `under_repair = true` → постройка доступна частично или полностью; поведение зависит от лора и механик мира
- Флаги снимаются через игровое действие (game action) по завершении работ

### 14.3 Статус

Детальная механика (время строительства, ресурсы, прогресс) — нет ТЗ; описывается в ТЗ по экономике.
