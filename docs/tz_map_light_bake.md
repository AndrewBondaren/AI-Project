...
`light_bake` / `full_bake` — **один** compose; отличается только набор macro-tiles.
...
**Сейчас (debt):** `prepare_surface_terrain_context` = coarse relief → hydrology сразу; light mountain = paint-only stub. Target: вставить **relief_objects_z** между ними; mountain contributor довести до z (M8 interim slice допустим).

**Имплементация (порядок слоёв):**

1. Coarse mountain/ravine z (declare + autoresolve footprint → rise/carve, clamp `z_min..z_max`)
2. Hydrology на этом heightmap (+ light-scale detect thresholds)
3. Light compose: sample elevated coarse + mountain paint/z
4. Полный FormGeometry→SideFill→KindElevation — следующим слоем, не блокирует (1)
