...
> **Impl:** новые ноды и wiring materialization — **отложены**; проектируем **отдельно с мастером**. До сессии — generators + debug API (`map.py`), см. [`tz_terrain_generation.md`](./tz_terrain_generation.md) § Phase 9+.

## Gate: DAG (жёсткое правило)

**Агент не правит** `application/engine/nodes/` (регистрация, deps, execute, wiring в container/DAG) **без явной совместной сессии с мастером.**

**Предусловие** — мастер закрыл **полное тестирование генераторов** (не «частичный smoke»):

| Gate | Содержание | Где в ТЗ |
|---|---|---|
| G1 | Terrain S→CL: `materialize-stack`, pole, hydrology, gap, column fill, climate overlay | `tz_terrain_generation.md` § D HY DoD, smoke |
| G2 | **TR-PERF** — bulk persist init принят (benchmark) | § TR-PERF |
| G3 | **TR-LAZY-LOAD** — service/repo (`get_scene_volume`, `has_column_cells`, z_slice persist) протестированы **через HTTP/debug**, без нод | § TR-LAZY-LOAD |
| G4 | TR-PAR determinism (workers 1 vs N) на реальном fixture | § TR-PAR-4 |
| G5 | Мастер явно снимает gate в чате / ТЗ | — |

**До gate:** target-спека нод ниже — **контракт на будущее**, не backlog агента. Реализация gameplay load — `MapCellService` + debug routes; ноды `check_terrain` / `eager_terrain` / `lazy_terrain` подключаются **после** G1–G5.

**Исключения (всё равно с мастером):** climate nodes уже в engine — не рефакторить «заодно»; новые ноды materialization (`generate_surface`, …) — только post-gate.
...
| id | phase | deps | Generator / service | Persist / load |
|---|---|---|---|---|
| `check_terrain` | pre_llm | `check_scene` | `has_column_cells` + `has_cells_for_location` (target § TR-LAZY-LOAD) | — |
| `eager_terrain` | pre_llm | `check_terrain` | `MapCellService.get_scene_volume` (target); сейчас `get_by_location` | read only |
| `lazy_terrain` | pre_llm | `check_terrain` | `generate_z_slice` → `save_pass` → `get_scene_volume`; fallback `generate_minimal` (§ TR-LAZY-LOAD) | upsert column / orphan |
...
| `POST …/map/materialize-stack` | `WorldSurfaceMaterializationOrchestrator` S→CL + `ctx` | target: gate + stack или цепочка нод с shared `ctx` ⬜ |
| `POST …/map/generate-surface` | `TerrainBatchOrchestrator` + `ctx` | `generate_surface` node ⬜ |
...
| `POST …/map/generate-climate` | `ClimateBatchOrchestrator` + `ctx` | `generate_climate` node ⬜ (миграция с `apply_climate_pass`) |
...
|---|---|---|---|---|
| `check_world_materialization` | pre_llm / gate | probe `free_cores` → `MaterializationContext` в `context` | — | world load / first player entry |
| `generate_surface` … `generate_climate` | post_llm | batch orchestrators + **shared `ctx`** (см. § MaterializationContext) | `MapCellService.save_pass` | world load / `full` init |
...
### MaterializationContext и `free_cores` (TR-PAR / CL-PAR)

> Утверждено вместе с [`tz_terrain_generation.md`](./tz_terrain_generation.md) § «Многопоточность (TR-PAR)». **Impl orchestrators ✅; DAG wiring ⬜ (TR-PAR-DAG-1).**

Параллелизм materialization (terrain chunks, climate batches) управляется **не** уровнями `DAGExecutor`, а **внутри** batch-orchestrators через `ChunkComputePool`. DAG обязан только передать **`free_cores`** один раз на run.

```mermaid
flowchart LR
  GATE[check_world_materialization]
  PROBE["free_cores probe"]
  CTX[MaterializationContext]
  TBO[TerrainBatchOrchestrator]
  CBO[ClimateBatchOrchestrator]
  GATE --> PROBE --> CTX
  CTX --> TBO
  CTX --> CBO
```
...
| Обязанность | Слой |
|---|---|
| Probe `free_cores = max(1, cpu_count() - reserve)` | **Gate-нода** materialization (или единый `materialize_surface` wrapper) |
| Собрать `MaterializationContext(free_cores=…)` | Gate; положить в `context["materialization_ctx"]` |
| `resolve_*_workers` + pool + serial persist | `TerrainBatchOrchestrator` / `ClimateBatchOrchestrator` — **не** в ноде |
| Переиспользовать **тот же** `ctx` на S→CL | Все materialization post_llm ноды одного run |
| World cap | `world.terrain_parallel_workers`, `world.climate_parallel_workers` — читает `ParallelPolicy` |

**Запрещено:**

- Каждая нода сама вызывает `os.cpu_count()` — только gate.
- `generate_climate` с `parallel_workers=1` hardcode, пока surface шёл с `free_cores=N` — один shared `ctx`.
- Подмена TR-PAR параллельным `asyncio.gather` на нодах `generate_surface` + `generate_ores` + … — ores/caves остаются serial passes; chunk pool только в terrain/climate orchestrators.

**Interim (код сегодня):**

| Caller | `free_cores` | Orchestrator |
|---|---|---|
| Debug HTTP `map.py` | stub `5` или query | `TerrainBatchOrchestrator`, `ClimateBatchOrchestrator`, `materialize-stack` ✅ |
| `GenerateClimateNode` | **нет** | `ClimateOrchestratorService.apply_climate_pass` — миграция на `ClimateBatchOrchestrator` + `ctx` ⬜ |
| `generate_surface` node | **нет** (нода не существует) | target: `TerrainBatchOrchestrator` + `ctx` ⬜ |

**Target gate snippet (контракт, не impl):**

```python
reserve = 1  # engine setting: event loop + HTTP
free_cores = max(1, (os.cpu_count() or 1) - reserve)
context["materialization_ctx"] = MaterializationContext(free_cores=free_cores)
```

### Запланировано (контракт нод)

#### Hydrology + procedural location names ([`tz_terrain_hydrology.md`](./tz_terrain_hydrology.md) U13)

> **Generator impl (D HY):** pure `HydrologyGeneratorService` + wire в `generate-surface` — **до DAG** ([`tz_terrain_generation.md`](./tz_terrain_generation.md) § D HY). Ниже — **DAG wiring** (H-7b, U13), после D HY-7a + ревью с мастером.

**Gate:** `world.hydrology.materialize_named_locations` / `world.caves.hydrology.materialize_named_locations` из **world template** — если `false`, цепочка **skip** (geometry без имён).

| id | phase | type | deps | Persist |
|---|---|---|---|---|
| `apply_hydrology` | post_llm | python | `generate_surface` | heightmap / connections (pure) |
| `collect_geography_naming_candidates` | post_llm | python | `apply_hydrology` | — |
| **`llm_name_procedural_locations`** | post_llm | **llm** | `collect_geography_naming_candidates` | — |
| `persist_procedural_locations` | post_llm | python | `llm_name_procedural_locations` | `NamedLocationService.upsert` + edge `location_uid` |
| `collect_cave_naming_candidates` | post_llm | python | `generate_caves` | — |
| **`llm_name_cave_locations`** | post_llm | **llm** | `collect_cave_naming_candidates` | — |
| `persist_cave_locations` | post_llm | python | `llm_name_cave_locations` | upsert cave geographic locations |

**Инварианты:** generators **не** вызывают LLM; master-declared locations **не** перезаписываются; debug HTTP path 2 **не** substitute LLM в product flow.

#### Climate ([`tz_climate.md`](./tz_climate.md) § три процесса)

| id | phase | Generator | Trigger |
|---|---|---|---|
| `generate_climate` | post_llm | `ClimateBatchOrchestrator.apply_climate_batch(..., ctx)` | ⬜ migrate from `apply_climate_pass`; **тот же `ctx`**, что `generate_surface` |
| `recalculate_climate` | post_llm | `recalculate(ClimateRecalcRequest)` | master patch; routing `ClimateChangeEvent` → request **в ноде**; **LOD:** near → per-cell bbox; far → skip cell upsert ([`tz_climate.md`](./tz_climate.md) C8, C11) |
| `resolve_weather` | pre_llm | `ClimateRuntimeAssembler.resolve_weather` | scene / tick; far → `location_weather` from snapshot |
...
| Дыра | Статус | Суть |
|---|---|---|
| **CITY-T-1b** dual persist | **open**, Gate: DAG | Debug: `SettlementOutdoorOrchestrator` → pack city + SQL-дерево + connections (C11). Нода `lazy_settlement` → `map_cells` `insert_bulk_ignore`. Игрок и harness видят разный город. Reuse freeze — **[CITY-T-5b](./tz_city_generation_technical_debt.md)** (**deferred**, lazy позже). Target wiring: нода зовёт **тот же интерфейс**, не internals orchestrator. |
...
- [`tz_lazy_simulation.md`](./tz_lazy_simulation.md) — LOD зоны near/medium/far
...
- [`tz_city_generation_technical_debt.md`](./tz_city_generation_technical_debt.md) — **CITY-T-5b** reuse freeze в интерфейсе generate (не нода)
...
| Дата | Изменение |
|---|---|
| 2026-09-06 | **CITY-T-5b:** reuse freeze — контракт generate, не Gate: DAG. Dual persist ноды остаётся 1b. |
