...
| REF-W index | ⬜ |
| `SCHEMA_ID` в 422 | ⬜ (целевое) |
| `worldSlices.py` реестр | ⬜ (целевое) |
...
Имплементация: `jsonValidation/index/` (⬜), **после** `resolve` + normalize.
...
Эталон: `WorldClimateScalars` + `climate_scalar_wire_from_mapping`.
...
- `precipitation_liquid` ∈ `material_registry` (REF-W)
...
| `WorldRegistryIndex` monolith | `index/` ⬜ поверх normalized POJO |
...
| JV-2 | REF-W index после normalize | P1 | ⬜ |
