---
name: tz-terrain-relief-v1-superseded
description: "Архив ТЗ relief grade v1 — не SoT generate. Bake R36u–w, R37 подробно, L2 volume. Актуальный generate+SQL+canal: tz_terrain_relief.md"
metadata:
  node_type: memory
  type: project
---
...
**Persist:** detailed / entry / patch upsert `relief_grade_instances` (`replace_world=False`). L0 light/full **не** `replace_world=True` — иначе wipe detailed outdoor grades. **Как** писать SQL — **R43** (bake-writer: одна txn + `executemany`, не commit на строку). Семантика upsert / merge `cell_refs` / T-3c systems — без смены.  
...
| R36t | **Bake formation anchors (SLOPE\|SHEER) — locked.** При **формировании** грани на bake всегда есть **верхняя** и **нижняя** точка перепада (якоря сайта / measured `dz`). Grade materialize + stamp **только между** якорями (коридор грани), **не** заливка региона. **Строго запрещено мутировать** клетки верхней и нижней точки (высота, материал, `system_grade_uid`, facing cache — не трогать якоря). **Не** правило entity R36j (состав объекта); это контракт **bake-формирования** (writer = **detailed_bake geometry**, R36u). **Исключение:** **canal** при **укорочении**, когда луч **не вмещается** — земля упирается в structure / obstacle footprint (`L_eff` < requested) — cut у укороченного конца по **R36p** (не knobs XOR на нормальном path, R28/R36q; нормальный `structure_canal` якоря не трогает). Без этой ветки якоря неприкосновенны. **Ещё:** L=1 первая downhill — **R41-T-15** (не этот canal-cut). **Запрещено:** трактовать бровку/дно как «весь объект = SHEER» только из‑за membership flood. Вершины/низины известны по готовому `heightmap` до paint — **R41** |
...
| R41 | **Relief pipeline v2 — locked (SoT).** Одна конструкция: вершина → кромка → фронты → `GradePaintSpec` → L2 paint. Контекст = тело вершины. Членство 8-связное при rim; клетка — **не больше одной** вершины (`at_grid`). `ReliefVertices` + **`occ`** + **`seam`**. **Один станок** после seed (8-flood + фронты + шов + pack). **Две очереди seed, не смешивать:** **Q1** leftover C39 / claimed тела; **Q2** производные (посадка SHEER, бок SLOPE-коридора, непокрытый сосед следа) в вёдрах `(z_q1, uid)`. Третьей очереди seed **нет**. Бок — свой слот/станок, **не** orphan: Instance входит в System ближайшего по \|Δz\| тела (**бок-attach**, не T-3c). Mill-код третьей очереди (`is_q3_seed` …) — снос **R41-T-19**. Каскад вёдер — **R41-T-18**. Фронт вниз пока θ (SLOPE); иначе **SHEER** к посадке; та же z = pack **COUPLE**. Не тело×8 как SLOPE/SHEER. Pack sidecar = **все** лучи фронтов по 8 сторонам + COUPLE на leftover + 8-halo, не «остаток». Луч = `(кромка, Facing)`; `(клетка, Facing)` уникален (**C41**). **1 фронт = 1 Instance;** System row при ≥2 **или** бок-attach. Каталог до пула; discover+paint → один fill; стык после чанков: прямая = catalog, вершина = T-3c на C29. Stamp `|dz|=1` — plains да / forest нет (R37 `stamp_min_abs_dz`). Ширина — **R42**. См. § Relief pipeline v2 · § Ray width |
...
| R44 | **Постгенерационный валидатор клеток рельефа — locked.** Образец: валидаторы структуры здания (log ERROR, не исключение, геометрия остаётся). Обход: leftover SLOPE/SHEER + 8-halo (`leftover_plus_halo`; зона вершин/коридоров/обрывов), **не** все 1e6 равнин тайла. Слот закрыт: в sidecar есть SLOPE / SHEER / **COUPLE** **или** нет соседа. Сосед другой z без SLOPE/SHEER → некорректна. Та же z без COUPLE в pack → тоже пустой слот (не закрывать из z). Лог только `relief_error` (тот же хелпер R8). Generate **не** abort. Валидатор **не** дописывает слоты. Контракт — [`tz_terrain_relief_consume.md`](./tz_terrain_relief_consume.md). **C43** |

### SQL catalog persist (2026-08-18)

| # | Решение |
|---|---|
| R43 | **SQL catalog = bake-writer, не CRUD-цикл — locked.** Один application-caller: `persist_relief_grades`. Геометрия по-прежнему pack column + `system_grade_uid` (R36u); SQL — каталог Instance/System, **не** dual-write сетки. **Как** [`tz_terrain_generation.md`](./tz_terrain_generation.md) TR-PERF-1 (`map_cells`): одна `Database.transaction()` на persist-pass, `executemany` пачками ~5000. **Не** N×`execute`+`commit` на строку. Prior merge — `list_instances_by_uids(world, uids_from_bag)`; **`list_instances_for_world` на этом пути запрещён**. Поштучный `upsert_instance` / `upsert_system` остаётся для patch/DAG (единицы строк; без обёртки — по-прежнему commit). Heartbeat: `packBakeLog` start/progress/done. Detailed: `generation_world_log(..., mode="detailed")`. Семантика `replace_world=False` / merge `cell_refs` / T-3c systems — без смены. Occupancy v1 не возвращать. Schema `0001` / DAG / discover/paint не трогать. **Не** бюджет «20–40 мин/tile» (мёртвый режим tile, WP-27 его нет). См. § SQL catalog persist |
...
| C15 | **Один угол / один фронт = один Grade (Instance).** Несколько сторон вершины или излом → System ≥2 Instance. **1 фронт** → строку `relief_grade_systems` **не** пишем (`Instance.grade_system_uid` NULL), **кроме** бок-attach Q2 (§ R41 очереди). Клетка → Instance, не System. Рабочий slot вершины ≠ строка БД | locked (R36l) |
...
| C24 | **Bake anchors:** верх/низ точки перепада при формировании SLOPE\|SHEER; **запрет мутации** якорей. **Исключения:** (1) **canal-cut** у укороченного конца, когда луч **не вмещается** — земля упирается в structure / footprint, `L_eff` < requested (**R36p**, не knobs на нормальном path); (2) **L=1** первая downhill — uid на нижней колонке (**R41-T-15**, не дырка 1×1). | locked (R36t) |
...
| C33 | **Вершина = одна псевдосущность.** 8-связные клетки той же z **с кромкой спуска** не сеют 8×N. Лучи только наружу с кромки. Rim-run / фронт с одним outward и тем же первым dz — один Grade. Повтор `(клетка, Facing)` между вершинами запрещён (C41). **SoT — R41.** Исторический R39 4-neigh — снят | locked (R41) |
| C34 | **Фронт = один θ до упора, смены угла или шва лучей (C41).** Швы чанков — `grade_uid` (R36w). Нижняя терраса — вершина **только если не покрыта** коридором сверху (C39). Open_land не продолжает L по полу низины. Излом θ → новый Grade (C15). `|dz|=1` stamp — plains да / forest нет (R37). **SoT — R41** | locked (R41) |
...
| C41 | **Шов лучей (не первый-занял).** Луч уникален по `(кромка, Facing)` на вершине и по `(клетка, Facing)` между вершинами. Несколько вершин могут стрелять в одну низину разными сторонами; повтор того же Facing в клетку — нет. Клетка ∈ ≥2 следов **одного** Facing → шов (шерсть). Клетка ∈ следов **разных** Facing (в т.ч. разных вершин) → якорь низа, ничей `occ`. Оба случая — один `seam[]` (C39 не сеет при `seam≠0`); два поля — later, если появится потребитель только якоря. Коридор = уникальные клетки следа. Пустой коридор → skip (дырка 1×1). Не 4 Instance на клетке. Не путать с C29. **Pack** (не Instance): **все** лучи фронтов (rim∪коридор × outward + получатель), в т.ч. SHEER к посадке Q2; плюс **COUPLE** same-z на leftover + 8-halo. Не тело×8 как SLOPE/SHEER. Не «остаточные» лучи. SLOPE/SHEER побеждает COUPLE. Получатель persist. Не второй обход тайла. [`tz_terrain_relief_consume.md`](./tz_terrain_relief_consume.md) | locked (R41) |
...
| C43 | **Постгенерационный валидатор 8 слотов.** После записи sidecar. Обход: leftover + 8-halo (`leftover_plus_halo`), не вся равнина тайла. На каждом Facing — pack-слот SLOPE / SHEER / COUPLE или нет соседа. Сосед другой z без SLOPE/SHEER → клетка некорректна. Та же z без COUPLE в pack → ERROR (не invent из z). **ERROR** через `relief_error` (`relief/log/log.py`), event `grade_cell_empty_ray`. **Не abort** generate (как structure headroom/staircase). Не чинит и не дописывает слоты. SoT — [`tz_terrain_relief_consume.md`](./tz_terrain_relief_consume.md) | locked |

---

## SQL catalog persist (R43)
...
**Инварианты (уже locked, не переоткрывать):** один application-caller `persist_relief_grades`; геометрия — pack column + `system_grade_uid`; SQL — каталог Instance/System, не dual-write сетки; schema `0001` не трогать; DAG/ноды не трогать; discover/paint не переоткрывать; occupancy v1 не возвращать; stamp → pack column не менять. **Бок-attach** пишет в **те же** `relief_grade_systems` / `Instance.grade_system_uid` (emit после T-3c UF) — § R41 очереди; не новая schema и не поле persist-POJO. Live bake-имя `side_parent_slot` — бок-attach, не очередь seed.
...
Поштучный `upsert_*` без `persist_session` **должен** коммитить — патч не ждёт bake-txn.

### Caller’ы (один helper)

| Caller | Когда | `replace_world` |
|---|---|---|
| `PackDetailedBakeOrchestrator` | offline L2, после чанков | `False` |
| `FineTerrainRefineOrchestrator` | entry / WP-13 | `False` |
| `PackMaterializationOrchestrator` | L0 light/full, если есть instances | `False` (**не** `True` — wipe detailed outdoor) |

### Наблюдаемость

Не print, не поллер `detailed_bake.py`. Logger `packBakeLog` уже в allowlist `generation_world_log` → uvicorn и файл, когда scope открыт.

Ритм как у чанков: не 32k строк, а пачка / несколько секунд. После `complete` в манифесте в логе должно быть видно `grades persist 0/N → … → done ~секунды`, а не тишина. Finish тайла (после чанков): `packBakeLog` **sidecar start/done**, **grade_ray validate 0/N → done** (`empty=`), **grade_systems emit start/done**. `app.relief grade_system_create` — DEBUG emit в памяти, **не** на консоль и **не** заменяет эти heartbeat. R44 ERROR — файл `gradeCellRays` + транскрипт, не stdout.

Entry/light/full уже внутри `generation_world_log` — те же start/progress/done попадут в `bake-entry|light|full-*.log` без новых обёрток.

### Ожидание / не обещать

Каталог на том же 1 км тайле (~25k instance) — **секунды, не 17 мин**. Не обещать «как чанки» (другой носитель). Обещать: не OLTP per-row.
...
Протокол repo → sqlite bulk+txn + get-by-uids → тонкий `persist_relief_grades` + `packBakeLog` → `generation_world_log` на detailed. **Не** «сначала только логи» и не «сначала только txn» в разных PR: без лога снова тишина, без bulk снова минуты.

Код ✅. Приёмка: N≫1 upsert = один COMMIT; поштучный upsert без обёртки коммитит; 10 новых uid → не `list_instances_for_world`; detailed log = start/progress/done + `elapsed_ms`; T-3c / persist-тесты зелёные; `map_cells` bulk не трогать.
...
**Код:** каталог граней до пула; `compute_rect` = discover+paint → один fill (R41). Occupancy v1 **удалён** (слой 7). Каталог `face_key` и L2 apply **живые**.  
...
**T-3c:** слой 6 плана v2 ✅ — emit `ReliefGradeSystem` при ≥2 Instance одной вершины. Intra-chunk = `slot`; через чанки = матч тел на C29 + UF **этого** refine. Не catalog uid = вершина. § T-3c на шве чанков.

#### Код v1 occupancy — срез слой 7 ✅

Live writer — `discover_and_paint` в `compute_rect`. Sample/stitch/`FineTileContext.planned` **удалены**. Исторические имена:

| Удалено | Замена |
|---|---|
| `sample_peak_land_sites` / `sample_downhill_land_sites` / `sample_landward_of_refs` | кромка C39 + plugin тела |
| `sample_open_land_meter` / `sample_shore_meter` / `sample_road_shoulder_meter` / `sample_ravine_meter` | discover в worker |
| `sample_detailed_grade_rect` / `plan_grade_for_rects` / `plan_rect_grade` | `discover_and_paint` в `compute_rect` |
| `stitch_planned_segments` (UF семян до пула) | стык **после** чанков: прямая = catalog uid; вершина = T-3c на C29 |
| `PlannedGradeSegment` / `FineTileContext.planned` | `GradePaintSpec` (C40) |
| `compute_rect` stamp `ctx.planned` | discover+paint на heightmap → один fill |

**Живые:** `TileFaceCatalog` / `face_key`; `plan_ribbon_volume` / `GradeFormation` / `DetailedGradeResult`; `grade_constrained` / `grade_from_template`; persist POJO/SQL; column fill после paint.
...
**Edge: два `detailed_bake`, одна лента (обязательный кейс / тест).** Хребет или склон **вдоль** ребра двух макро-тайлов — безшовная геометрия мира, технически **две** джобы (`scope=wilderness` по тайлу, location vs wilderness, entry vs offline). Джобы **не** ждут и **не** лочат друг друга. Сшивка **прямой** = тот же catalog uid + persist `replace_world=False` (upsert `cell_refs`). System через этот макро-шов — **не слой 6** (T-3c = чанки **этого** refine).
...
Общая грань двух чанков: **один** catalog `face_key` (SoT identity шва). Discover/paint шва — **owner-чанк**; сосед только штампует тот же uid. Ни один не владеет гранью как своим grade uid.
...
**Граф** (узлы = уже выданные face uid; рёбра **не** создают uid) — **C28 T-3b ✅**. Черновик «компонента = один Instance + min(face_key)» **superseded**: компонента = топология склона; прямые режутся отдельно; rim-canonical — **C29** (один объект мира на двух джобах), не продукт «шов». System ≥2 прямых **одной вершины** — **T-3c** (тело на C29, не catalog = вершина). Полный контракт — § Topology → entity → stamp · § T-3c на шве чанков.
...
FineChunkRunner.refine_rects                    # оркестратор; не god-method
  ...
  merge instances                               # C29: один catalog uid → один Instance (upsert cell_refs)
  T-3c на швах + persist systems                # slot intra-chunk; тело на C29; UF тайла; System iff ≥2
```

**Код сейчас:** discover+paint в `compute_rect`; occupancy v1 срезан (слой 7). **T-3c:** `FineChunkPersist.finish` → `emit_relief_grade_systems` после `merge_grade_instances`; persist `instances`+`systems` через `persist_relief_grades` (**R43**, не per-row CRUD).
...
|---|---|---|---|
| **`FineTileContext`** | frozen dataclass | prep + catalog → compute/persist: `surface_state`, `catalog`, `grade_halo`, `existing_uids`, `templates`, bbox, workers | persist; mint uid |
| **`prepare_fine_tile`** | один helper до пула | parent light, surface, halo + `refresh_tile_gaps`, catalog, existing uids (тот же `WorldPackReader`), workers | discover/stamp; pool |
| **каталог** | serial до пула: **только `face_key` / job uid** (C28) | identity граней; uid шва заморожен | materialize; fill; discover по z (это **в worker**, R41); persist; mint; System row |
...
| **`FineChunkPersist`** | write lock здесь | partition, wilderness write, location flush, grade_acc, counters | generate; второй grade pipeline; per-row SQL catalog (SQL — **R43** после чанков) |
...
Параллель: два чанка, общая грань — **один** catalog uid (одна **прямая**); discover один раз (owner). Lock на mint не нужен. Стык рельефа — **после** чанков (R41): catalog = прямая; T-3c = тело вершины на том же C29. Не union-find семян до пула. Не полный same-z CC тайла.
...
| Можно | Нельзя |
|---|---|
| Каталог граней + uid **до** пула; общая грань = один uid; discover шва = owner-чанк | Mint uid в воркере; два discover одной грани в одном вызове |
...
| System из 1 прямой; клетка → `grade_system_uid` | R36l (исключение: бок-attach Q2 — § R41 очереди; клетка всё равно → Instance) |
...
**Код сейчас:** R36w каталог (живой); `compute_rect` = discover+paint → один fill (R41). Occupancy v1 срезан (слой 7). **T-3c слой 6 ✅.** `_plan_tile_grade` убран.
...
**Связь с R36t / C24:** canal-cut при укорочении — **валидное** исключение из «не мутировать якорь», когда земля **упирается в structure** / footprint и `L_eff` < requested (якорь у этого конца может быть в cut). Не снятие якорей вообще. Не T-15 (L=1 stamp). Нормальный path (места хватает) якоря не трогает: `canal_obstacle_policy` игнор; knobs `structure_canal` — lined ditch на вместившейся грани, не cut якоря.
...
# очереди seed раздельно (R41): Q1 leftover C39 → Q2 производные (посадка / бок / сосед следа); станок тот же
# бок: свой слот; Instance в System ближайшего по |Δz| тела (не orphan); очереди Q3 нет
# не OR предикатов Q1 и Q2 в одном проходе
...
**`geom_resolve` / `partition_height` / R36i inner не меняем.** После clamp plains L knobs ≥ `ceil(h/tan 45°)` и ≥ 20, если луч длиннее; иначе L = L_ray. Inner по-прежнему `L_eff = min(L,h)` (без плоских хвостов на unconstrained path).
...
**Plains (locked, `open_land` only):** `apply_in_contexts = (open_land)`. Клетка кубическая: `tan θ = h/L`. **Каркас вершин/фронтов — R41 / C39** (не R38–R40). Нижняя терраса — вершина **только если не покрыта** коридором.

| | |
|---|---|
| Первый шаг `|dz| = 1` (4→3 / 3→2) | **stamp** (`stamp_min_abs_dz=1`). Кромка C39. Не boolean на plugin. Дырка `4 3 4` (одна клетка) — skip C41. Чаша шире — стороны на уникальных клетках кольца, пол низины не `occ` |
| Первый шаг `|dz| ≥ 2` | lockstep (R41): стоп если нет клетки, **упор** (R36m), **смена θ**, своё тело / чужой occ/seam; **подъём** `z > z_body` или `z > z_prev`. **Равная z = продолжение L** только ravine / `grades_channel_bed`; open_land пол низины — стоп. Клетка излома сеет только если предикат C39 |
| L_ray | число свободных клеток луча (включая seed); шаг = Chebyshev 1 (R36s) |
| h | `z_crest − z_end` |
| θ max | **45°** (было 20°). θ < 20° на длинном луче **по-прежнему SLOPE** (L min 20). Новая полоса — короткие ступени **20°–45°** |
| L min | **20**, если луч позволяет; **без L max**. Короткий луч: L = L_ray, классификация по θ (не veto L < 20 → SHEER) |
| L knobs | `L = min(L_ray, max(L_tpl, ceil(h / tan 45°), 20))` |
| влезает | `atan(h/L) ≤ 45°` → SLOPE |
| не влезает (напр. 4→2, L=1, ≈63°) | SHEER (`L=1`) |
| `allow_l_gt_h` | **true** |

Примеры: h=4, луч длинный → L=20, θ≈11°; h=10 → L=20, θ≈27°; h=1, L_ray=1 → SLOPE 45°. 4→2 при L_ray=1 → SHEER.

Deprecated sample `measure_terrain_descent` still stops on `z ≥` (R40). Live lockstep is **R41** `z >` (equal z continues L).

**Forest (locked, `open_land` only):** θ≤**20°**, L min 20 без крышки, `allow_l_gt_h`, `stamp_min_abs_dz=2` (единица **не** stamp — не как plains). **Каркас — R41.** Отличие kind: **SHEER допустим**, если `|dz| ≥ sheer_min_abs_dz` (**4**) — тогда kindRoll может выбрать отвес; L SHEER всегда **1**. `|dz| < 4` (и луч позволяет 20°) → только SLOPE. Не влезает в 20° на L_ray → SHEER L=1.

**Shore classes (locked, `shore` context only):** четыре строки конверта. Как plains vs forest: один контекст, разные полы. **Не** копировать plains (θ≤45°, L min 20).
...
**Порядок на ядре (v1, не выполнять — SoT R41)**
...
Суть: готовый `heightmap` чанка → вершины с кромкой спуска → фронты (схождение режет **шов лучей**, C41) → Grade только в уникальном коридоре → **один** column fill (parent⊕overlay) → стык **уже сформированных** чанков: catalog uid = одна прямая; T-3c = одно тело вершины на колонке C29. Не рельеф тайла параллельно с чанками и не fill∥relief. **Расписание A locked** (не путать с caller A = `FineChunkRunner` vs patch). Не путать post-chunk C29 с `SeamStage` (C41).
...
**Pack 8 слотов — occupancy не закрыт (R41-T-25 P1).** SoT угла/стрелок — [`tz_terrain_relief.md`](./tz_terrain_relief.md) (местность θ ∈ **(0, 90)**, **90° нет**; SHEER **[80, 90)**; нависание — отдельные объекты, не leftover; `|dz|=2` ≈63° — SLOPE). T-17 ✅: leftover + COUPLE, валидатор не из z. Алгоритм записи и новый валидатор — следующая разработка. Не игнор восьмёрки (dump → `_`). Interim `downhill_leftover_rim_rays` / `slope_outcome(L=1)` порог 45° → SHEER — не SoT.

**Вёдра Q1/Q2 ✅ (R41-T-18 / T-19).** Канон: [`tz_terrain_relief.md`](./tz_terrain_relief.md) § Две очереди seed. Третьей очереди seed нет. Бок-attach persist жив (`side_parent`).
...
|---|---|---|---|
| 0 | **Алгоритм полного заполнения 8 слотов pack** | **R41-T-25** P1 | игнор всех 8 → dump `_`; валидатор skip; тело×8 Instance; считать T-17 / `downhill_leftover` конечным SoT |
| 0b | Pack leftover → COUPLE; валидатор не из z | **R41-T-17** ✅ | leftover-only `walk_pack_senders`; invent `+` в dump из z; валидатор закрывает same-z из z; last-wins затирка |
| 1 | Вёдра Q1 leftover/claimed + Q2 `(z_q1, uid)` | **R41-T-18** ✅ | повторный walk rect; третья очередь seed; clear очереди сбросом mill; снять бок-attach persist |
| 1b | Снос mill-очереди Q3 (`is_q3_seed`, `q3_s`, `q3_parent`) | **R41-T-19** ✅ | снять бок-attach persist; `0002_*.sql` |
| 1c | Полиш mill после T-18/T-19 | **R41-T-20…T-23** P2 ✅ | склеивать Rim/Front/Seam; третий **очередь** seed; снять бок-attach |
| 1d | `z_at` → `z_height_map` (метод, Mapping, Callable, consume TZ) | **R41-T-24** P3 ✅ | смешивать с T-17; `ParentLightTile.surface_z_at`; `terrain_z_at` city |
| 2 | Полиш луча и L | **R41-T-5…T-8** ✅ | не возвращать pick в `FrontStage`; не ставить envelope floor как occupancy cap |
| 3 | Тела plugin | слой 5 ravine ✅; shore онтология+paint ✅; **ShorePlugin тело ✅** | не Priority-Flood; не `if context` в `core.py`; не хардкод 2/5/20/70 |
| 4 | System row | **T-3c** / слой 6 ✅: ≥2 Instance одной вершины; intra-chunk = `slot`; через чанки = тело на C29 + UF этого refine | uid System на клетке; строка при 1 фронте; второй C39; C41; `face_key` = вершина; ждать соседний `detailed_bake` |
| 5 | Срез v1 | слой 7 ✅: sample / `plan_grade_for_rects` / stitch / `FineTileContext.planned` удалены | возвращать occupancy v1 |
| 6 | Полиш facade | **R41-T-9…T-12** ✅ | склеивать Rim/Front/Seam в один `core.py` |
...
| `|dz|=1` чаша 1×1 | locked skip C41; не четыре uid. L=1 одиночное ребро штампует первую downhill — **R41-T-15** ✅ |
| Равная z / C41 полиш | **R41-T-13…T-16** ✅ |

**Запрещено из «осталось» выводить:** второй пул; fill∥relief; два column fill; полный same-z CC тайла; stamp uid на тело вершины; попарный emit System на каждом ребре без UF тайла; повторный discover на шве.
...
Компонента 8-связных клеток той же целой z попадает в `members` **только если есть кромка спуска** (хотя бы один 8-сосед ниже). Интерьер без спуска входит в тело, не сеет. Плоский двор без перепада — **не** вершина. Низина / подножие — якорь низа, не тело. Клетка в `at_grid` — **не больше одного** слота. Пол ямы не член верхней месы. Не каждая клетка чанка — вершина.
...
ReliefVertices
  ...
  seam:    array[int]           # w*h; 0 нет; иначе шов лучей (C41; не коридор, не тело)
  facing_bits: array[uint8]     # w*h; бит на Facing; claim (клетка, Facing) между вершинами
  members: list[dict[(x,y), z]] # slot-1 → клетки тела; dict, не list
  uids:    list[str]            # slot-1 → grade_system_uid (пусто, пока нет System row)
```

**Запрещено на hot path:** `dict[(x,y), uuid]`; list клеток в `members`; uuid в сетке. `idx_of` — только если uuid уже есть до persist. **Не** хранить `ReliefGradeInstance` / `ReliefGradeSystem` в `members`. **Не** расширять persist-POJO полями `occ` / `seam` / `facing_bits` / slot / фронт.

**Слои (не смешивать)**

| Слой | Тип | Зачем |
|---|---|---|
| Индекс bake | `ReliefVertices` (application dataclass, **не** `dataModel`) | слоты, `occ`, `seam`, `facing_bits`, тело `dict[(x,y), z]` |
| Объект построения фронта | **`GradePaintSpec`** (тоже application; поля — **этот** пункт, C40) | единственный вход в L2 |
| Persist / wire | `ReliefGradeInstance` / `ReliefGradeSystem` (`dataModel`) | SQL + клетка `system_grade_uid` |

Построение: слот + кромка → `GradePaintSpec` (decision с POJO) → apply → `ReliefGradeInstance(...)`. Коллекция не «портит» Instance и не держит вторую копию h/L.
...
| `seam` | нет шва лучей | клетка встречи ≥2 следов / ≥2 Facing (C41, в т.ч. разных вершин); не коридор |

Тело вершины **не** ставит `occ` и **не** ставит `seam`. Пол/якорь низа **не** сеет; шов лучей в `occ` не попадает.

**Предикат seed (незатопленная кромка):**

`occ[i]==0` ∧ `at_grid[i]==0` ∧ `seam[i]==0` ∧ ∃ 8-сосед `n`: `z[n] < z[i]` ∧ `occ[n]==0`

**Алгоритм (SoT, не Priority-Flood ямы):**

1. Один проход rect+halo: клетки в Q1 leftover `(z, unset)` (целая высота, counting/bucket — не heap). Дальше сетку не перечитывать.
2. Расписание seed — § [Две очереди seed](#две-очереди-seed--общий-станок-locked-r41): за цикл только `z_top`, C39 из leftover этого z, mill, Q2, снять z.
3. 8-flood тела — **только от** посеянной кромки, та же целая z, через `occ==0` ∧ `seam==0`. Интерьер без спуска входит в тело, не сеет новые фронты.
4. Фронты с кромки — общий станок (луч до упора). Излом / упор / шов → этот Grade кончается.
5. Ниже по z занятые коридором отваливаются на `occ`. Непосеянный leftover остаётся в своём ведре Q1 до своего `z_top`.

Не второй линейный проход rect «найти хвосты». Это **C39** (независимые пики в leftover Q1), не leftover pack.

##### Две очереди seed + общий станок (locked, R41)

Тот же `ReliefVertices` (новые **слоты**, не второй dataclass и не leftover-глифы).

**Общий станок** (после любого seed, не копировать): `plugin_for` → `RimStage.flood` (8, та же z) → `add_vertex` → `FrontStage.propose` SLOPE (сначала отсев занятого `(посадка, Facing)` и заведомого SHEER на первом шаге `atan(|dz|, 1)`; длинный lockstep только у оставшегося SLOPE) → `SeamStage.commit` → после ZQ `propose_sheers` / `finalize` → pack: **все** `rim ∪ corridor × outward` (+ получатель) **и** COUPLE same-z на leftover + 8-halo. Envelope / template (`slope_outcome`, knobs) и C41 — те же. Равная z = pack **COUPLE**, не invent в dump. Клетка коридора SLOPE (след фронта после `commit`; `occ`, если C41 уже поставил) **не** тело новой вершины.

**Расписание seed (locked, R41 / T-18).** Mill: один walk leftover, цикл `z_top`, Q2 из событий станка. Канон ниже, не в registry.

Bake-`uid` = слот в `ReliefVertices`, не SQL `grade_system_uid` (R36t). **N** (`stamp_min_abs_dz` / envelope) — только stamp фронта, не сколько z класть в leftover Q1 за цикл.

**Вёдра = очереди.** Один walk в leftover Q1. `ReliefVertices` / `occ` / `seam` живут до конца discover. Отдельный clear очереди **нельзя** (не обнулять `at_grid`).

| Семья | Ключ | Имя (лог) | Содержимое |
|---|---|---|---|
| Q1 leftover | `(z, unset)` | `{z}_0` | ещё не сеянные клетки этой высоты |
| Q1 claimed | `(z, uid)` | `{z}_{uid}` | тело уже посеянной вершины |
| Q2 | `(z_q1, uid)` | `{z_q1}_{uid}` | производные **этой** вершины; `z_q1` — высота родителя Q1, не z клетки посадки/бока |

Сортировка: `z` убывая, затем `uid` (не строка `"10_…"`). **Клетка ∈ не больше одного ведра** на обе семьи. Обратный индекс `клетка → ключ`; дубль при вставке — отказ.

**ZQ** — z с непустым leftover Q1.

**Цикл, пока ZQ не пусто** (повтор с пункта 1):

1. `z_top = max(ZQ)`.
2. Q1: только leftover `z_top`, предикат C39. Занятые skip.
3. Mill. С 8-взгляда кромки **сначала** выкинуть: занятый `(клетка посадки, Facing)`; первый шаг, который конверт уже кладёт в SHEER/skip (`slope_outcome(|first_dz|, L=1)` vs `slope_max_angle_deg` / `stamp_min_abs_dz` этой клетки, не литерал 45°). **Потом** SLOPE lockstep W×L (C34/R42) **до упора**. Не ходить длинным SLOPE в занятый луч и не маскировать обрыв под L>1. Occupancy / `L_tpl` как сейчас. Не резать луч на ступени z. Не сеять продолжение той же прямой новой вершиной. Коммит SHEER-следов (`propose_sheers`) — после ZQ, чтобы Q2-посадка успела сеять (не `occ` с L=1 до drain).
4. Тело → Q1 `(z_top, uid)`. Посадка и бок (в т.ч. непокрытый сосед следа = тот же бок) → Q2 `(z_top, uid)`.
5. Drain Q2 только с `z_q1 == z_top`, пока не пусто. Предикаты посадки / бока раздельно, не `OR`. Новая производная → Q1 claimed `(z_клетки, new_uid)`; её порождения — Q2 той же волны `z_top`. Бок → бок-attach persist (ниже).
6. Leftover Q1 на `z_top` пуст и Q2 на этот `z_q1` пуст — снять `z_top` из ZQ (даже без seed). Claimed Q1 не стирать.
7. Если ZQ не пусто — пункт **1**.

Нижний leftover, которого луч не тронул, ждёт в Q1 `(z_ниже, unset)`. После ZQ пусто: `propose_sheers` / finalize / reconcile — не очередь seed.

Impl-тикеты: [`tz_generator_technical_debt.md`](./tz_generator_technical_debt.md) **R41-T-18** (вёдра), **R41-T-19** (снос `is_q3_seed`). Полиш слоёв — **R41-T-20…T-23** ✅.

**Две очереди — разные seed, не смешивать.** Не `OR` предикатов в одном проходе. Клетка, уже `at_grid`/`occ`/`seam` или уже в другом ведре, в следующую очередь не входит.

| Очередь | Seed (только этот предикат) | Не есть |
|---|---|---|
| **Q1** | leftover: **C39** (свободно + 8-сосед **ниже** с `occ==0`); claimed: тело после mill | посадка без своего спуска; бок рампы (это Q2) |
| **Q2** | производные вершины Q1: посадка SHEER к телу; бок same-z SLOPE-коридора. Ведро `(z_q1, uid)` | C39-пик leftover; пол рампы (клетка следа); продолжение той же прямой как новая вершина |

«Непокрытый сосед следа» в этом каноне = тот же предикат, что бок same-z SLOPE-коридора (`is_side_seed`): не отдельный kind и не третья очередь seed.

Коридор SLOPE = клетки закоммиченного SLOPE-фронта. `occ` на клетке есть, если это не локальный мин (C41 пишет мин на finalize). Сосед бока Q2 — эта клетка коридора. Сам пол рампы не seed, даже при `occ==0` (`|dz|=1` plains). Не ждать марку `occ`, чтобы увидеть бок.

**Бок (Q2) и идентичность Grade (бой / LLM) — locked.** Бок — **новый слот** и тот же станок (тело на своей z, свои фронты). Это **не** orphan Grade и не leftover «тело×8». После станка Instance бока входят в **`ReliefGradeSystem` ближайшего по высоте** уже существующего Grade:
...
3. Если у выбранной вершины ещё нет System (один фронт, `grade_system_uid` NULL) — **создать** System: Instance родителя + Instance бока. Исключение из «1 фронт → нет строки System» (R36l / T-3c): только этот attach.
...
**Бок-attach → persist (locked).** Это **тот же** каталог Instance/System (R43). Не новая таблица, не новая колонка, не `0002_*.sql`, не dual-write сетки.
...
| След воркера | то же семейство, что T-3c (`VertexSlotSeam` / аналог): slot + uid Instance слота + тело на ребре **и** parent слота бока | колонка SQL; `occ` / slot / фронт на Instance (C38) |
| SQL | `relief_grade_systems` + `Instance.grade_system_uid` у **всех** членов компоненты | parent_slot; флаг «это бок»; uid System на клетке |
| Клетка / pack | `system_grade_uid` = **Instance** как сейчас (R36l / C11) | uid System |

Bake-поля `side_parent` / `side_parent_slot` — бок-attach; поведение attach без третьей очереди seed.
...
2. Бок-attach: union ключа слота бока с ключом **parent в том же чанке**. Не матч тела, не same-z. Если parent уже в компоненте T-3c (меса на нескольких чанках) — транзитивность: бок входит в ту же System.
...
Если у parent нет нарисованных Instance (нет следа слота) — union невозможен; бок остаётся 1 фронт без System, пока у него самого нет ≥2 Instance. Бок-от-бока: цепочка child→parent в том же emit, если у каждого есть след.

**Запрещено:** колонка parent / slot в SQL; stamp `grade_system_uid` на клетку; mint System в воркере до UF; persist bake-слота; смешивать `members` разной z; второй caller persist; менять schema `0001`.
...
##### Шов лучей (locked, C41)

Не путать с техническим швом чанков/тайлов (**C29** / R36w `face_key`).

Клетка принадлежит **не больше чем одной** вершине (`at_grid`). Клетка коридора — **не больше чем одному** фронту (`occ`). Луч с кромки — **не больше одного на `(клетка кромки, Facing)`**. Между вершинами — **не больше одного на `(клетка посадки, Facing)`**: вторая вершина не стреляет той же стороной в ту же клетку. Это не запрет сходиться лучам с **разных** сторон: низина может принимать склоны с N и с E **от разных вершин**; центр ничей.

**Не** «непересекающиеся фронты». **Не** первый-занял (кто раньше написал `occ` — тот съел низину). **Не** «одной низине — один склон».

**Алгоритм (на одну вершину, до записи `occ`):**

0. 8-взгляд с тела: не стрелять в себя / same-z leftover; не в занятый `(посадка, Facing)` и не в чужой `occ`; вид с `slope_outcome(|first_dz|, 1) ≠ SLOPE` не идёт в длинный walk (SHEER/skip).
1. У каждого оставшегося SLOPE-фронта — предложенный след: lockstep W×L (R42), стоп = тело этой вершины, излом θ, упор (R36m), чужой `occ` **других** вершин, **или** (open_land / обочина) плоский шаг `z == z_prev`. Чужой `seam` низины на первом шаге — якорь, не запрет выстрела.
...
Чужой `occ` (каскад сверху, интерьер склона) — обычный упор следа. Чужой `seam` низины seed не режет (C39).
...
`a` — уникальный коридор стороны (open_land не идёт по полу `2`); `·` — шов разных Facing (угол кольца). Центр низины не `occ` и не `seam` (луч до него не дошёл). Не четыре uid на клетку.
...
Четыре (и до восьми) следов, единственная `3` ∈ всех → шов, пустые коридоры → skip. Между кромкой и якорем нет клетки. Это **не** повод отдать `3` первому лучу и **не** «один склон на низину». Plains stamp `|dz|=1` на **одиночном** ребре это не отменяет.

**Запрещено:** четыре `cell_refs` на одну клетку; класть пол/шов в `members` верхней месы; сеять вершину со шва (как с пола рампы); первый-занял внутри вершины; путать шов лучей с C29.
...
**Одна грань = один фронт = один Instance** (один θ, один `Facing`). У вершины может быть 1–8 фронтов. **1 фронт → строки `relief_grade_systems` нет**, **кроме** бок-attach: бок и родитель входят в один System даже если у каждого по одному фронту (§ R41 очереди). SQL FK: `grade_system_uid` на Instance ссылается на `relief_grade_systems`; NULL допустим, пока нет строки. **Не** писать uuid на Instance без строки System. Рабочий слот у холма с одной стороной есть; persist System — при ≥2 Instance **или** бок-attach.

##### `GradePaintSpec` (шов L2) — поля locked (C40)

Единственный вход в materialize. Discover **не** пишет `z`/`uid`. `plan_ribbon_volume` остаётся 1D; повторить `z[k]` по ширине фронта.

**Не** persist-POJO и **не** дубль Instance. Kind/h/L/θ/canal **не** объявлять здесь повторно: только ссылка на уже решённый `RibbonGradeDecision` (он читает `ReliefGradeKnobs` / envelope).

| Поле | Тип / смысл |
|---|---|
| `grade_uid` | uid **этого** фронта → станет `ReliefGradeInstance.grade_uid` |
| `outward` | `Facing` коридора |
| `front_w` | ширина lockstep (R42); орто W×L; diag day-1 = 1 |
| `anchor_top` / `anchor_bottom` | якоря R36t (не мутировать). При схождении фронтов `anchor_bottom` может быть **общей** клеткой шва лучей |
| `decision` | `RibbonGradeDecision` (kind, h, `requested_length`, `geom`, canal knobs) |
| `corridor` | уникальные клетки полосы **между** якорями (occupancy); **не** шов лучей (C41), не тело вершины |

Apply: `ReliefGradeInstance` из POJO + `cell_refs` = коридор (T-12). `grade_system_uid` на Instance — только если есть строка System (`uids[slot]`).

Day-1: орто W×L + диагональ W=1 Chebyshev `(±1,±1)` через существующий `outward_columns`. Толстый диагональный фронт — R36r later.
...
| `open_land` plains | `L = min(L_ray, max(L_tpl, ceil(h/tan 45°), 20))` — шаблон 2 **поднимается** на длинном луче, не «режется до 2»; короткий луч: L=L_ray, θ≤45° → SLOPE |
| `open_land` forest | `L = min(L_ray, max(L_tpl, ceil(h/tan 20°), 20))` |
...
До 8 сторон с кромки, только **вниз** и не в то же тело. **Одна клетка кромки × один `Facing` = один луч** (пик может смотреть все 8; меса не стреляет диагональ в орто-посадку соседа). Между вершинами `(клетка посадки, Facing)` уникален. У стороны свой θ до упора, шва лучей (C41) или пока z не сойдёт с рампы. **Равная z вдоль луча = продолжение L** только если `envelope.grades_channel_bed` (канон ravine + берег канала). `FrontStage` не ветвит `if context`. Open_land / обочина: плоский пол низины **не** коридор — стоп при `z == z_prev` (k>1). Стоп на **подъёме** `z > z_body` / `z > z_prev` (R41-T-5). **Ширина — R42**. Схождение следов: один Facing → шов-шерсть; разные Facing (и с разных вершин) → якорь низины, не first-wins. Излом → новый Grade / кромка **если клетка ещё не покрыта** и не шов. Каскад сверху вниз по z. Stamp `|dz|=1` на plains — **да** (R37, `stamp_min_abs_dz=1`); forest — нет (`=2`). Диагональный thick volume — R36r later; шаг = Chebyshev 1 (R36s).
...
| Sample семян до пула; стык = каталог | Каталог = ключ **прямой**; discover в worker; **стык после чанков** (прямая = catalog; вершина = T-3c на C29) |
...
| `|dz|=1` «не вершина» | `|dz|=1` вниз — кромка; plains **stamp** (R37 `stamp_min_abs_dz=1`); forest — нет (`=2`); дырка 1×1 — skip C41; чаша со сторонами — уникальные клетки кольца, не один склон на низину |
| Дорога = отдельный ribbon v1 | Полотно = одна вершина; поворот 8-way; внутренний угол = шов лучей (C41) |
...
На орто (`N/E/S/W`) Bresenham не нужен: коридор = прямоугольник `W × L`. На диагонали day-1 тот же lockstep, **W=1** Chebyshev `(±1,±1)`. Murphy thick W>1 — **R36r later**, не day-1.
...
Угол месы: север и восток — два фронта. Пик 1×1 может смотреть все 8. Диагональ **не** садится на клетку, которая уже орто-спуск **другой** клетки того же тела (иначе шов съест орто-фронт).

Stamp `|dz|=1`: фронт-кромка есть, объём грани нет (R37).

**Запрещено:** 8 ниток **как модель ширины** (вместо R42 W); веер в низину; dilation/buffer как SoT ширины; менять якоря кромки и низа. До 8 **направлений** с одной клетки кромки — да.
...
| **Generate** | `RavinePlugin`: банк + стены маски; стрельба в маску |
...
| **Generate** | `ShorePlugin`: банк + полоса; дно iff envelope `grades_channel_bed`; leftover terrace iff `sheer_terrace_min_cells`. Stepped SHEER volume — later |
...
| `1 ≤ free_gap < L_req` | obstacle clearance (R36m): укоротить L; **open_land R37** после луча: plains `atan(h/L) > 45°` → SHEER L=1; forest `> 20°` → SHEER L=1 |
...
| Конверт R37 (plains `open_land`) | Discover: кромка × фронт (R41). Pick: `L = min(L_ray, max(L_tpl, ceil(h/tan 45°), 20))`; `atan(h/L) > 45°` → SHEER L=1. R36m obstacle — отдельно, после |
| Конверт R37 (forest `open_land`) | как раньше: `ceil(h/tan 20°)` / L min 20; overflow 20° → SHEER L=1 |
...
| **Исключение canal** | луч **укорочен**, потому что земля **упирается в structure** / footprint (`L_eff` < requested) → cut у этого конца по **R36p**. Живое исключение, не «устарело». Нормальный path (места хватает) и knobs `structure_canal` якоря **не** трогают |
| **Исключение L=1** | одиночный downhill: коридор = **первая клетка спуска** (не дырка 1×1 с пустым коридором C41). Stamp uid на эту нижнюю клетку **разрешён**. Не canal-cut и не четыре uid на общую `3`. Верхняя кромка без uid (**C11**) |
...
Номера ниже — **исторические вопросы entity** (R36j/m), **не** очереди mill. Mill Q1/Q2 — § [Две очереди seed](#две-очереди-seed--общий-станок-locked-r41) / **R41-T-18**.

| # | Вопрос | Черновик default |
|---|---|---|
| **R36j-A** | ~~angle field on cell~~ | **superseded:** angle на **Grade entity** (`angle_deg`) |
| **R36j-B** | ~~L+angle on cell~~ | **locked R36j:** Grade entity + `system_grade_uid` на клетке; h/L/angle **не** на клетке |
| **R36m-A** | ~~Expand → building/road~~ | **locked R36m/n + R36p/q:** clearance = `L_eff`; knobs XOR canal; policy only if не вмещается; `canal_template_registry` |
...
Sink файлов — [`tz_logging.md`](./tz_logging.md): консьюмеры [`relief` / `reliefLog`](./tz_logging.md#каталог-консьюмеров) (этот раздел) и [`relief` / `gradeCellRays`](./tz_logging.md#каталог-консьюмеров) (R44 / consume). Не общий `app.log`.

| Уровень | Что писать |
|---|---|
| **INFO** | template_uid, context, sides/kind summary, identity; heartbeat консоли — `packBakeLog` (chunk / sidecar / validate / T-3c emit), не per-system |
| **ERROR** | постгенерационный валидатор (**R44** / **C43**): `relief_error` + `grade_cell_empty_ray`; generate не abort. Файл `gradeCellRays` + транскрипт; **не** консоль (шторм). Счётчик `empty=` — в heartbeat validate |
| **DEBUG** (sample) | `kind`, `h`/`L`/`angle` (R36) или `t`/`Δz`/`fraction`, **`reason`**, **`facing`** или `facing=none`; **`grade_instance_create`** — базовые поля Instance; **`grade_system_create`** — `why` + uid/kinds/angles; **`grade_system_members`** — детали частей; **R36w:** `detailed_grade_sample_rect` / `detailed_grade_materialize_rect` / `grade_uid_inherit` + **`cpu_core`** / `worker_thread` |
...
pack/refine/
  detailedGradeGenerate / detailedGradeDiscover / detailedGradePaint / detailedGradeMaterialize  # R41 discover+paint; L2 write-set
  detailedGradeCatalog     # R36w face catalog + job uid keys
  meterGradeSurface        # READ z; write-set = DetailedGradeResult.surface_z ✅
  fineChunkRunner          # one pool: discover+paint+fill per ColumnRect (**R41**) ✅

application/worldData/persistReliefGrades.py  # единственный caller SQL catalog; **R43** bake-writer ✅
...
каталог face uid                    # identity ребра / job; уже есть (R36w); до пула
  ...
  → после чанков                    # (1) catalog: одна прямая = один Instance
                                    # (2) T-3c: intra-chunk slot; тело на C29; UF тайла
                                    #     ≥2 Instance одной вершины → System row
```
...
**Код T-3b occupancy:** удалён слой 7. Rim-canonical = `catalog.uid_for_faces` в `discover_and_paint`. Каталог граней — живой. System — T-3c, не occupancy-stitch.
...
|---|---|---|---|
| **Grade** | одна **прямая** = один Instance (rim-canonical + upsert `cell_refs`); одна **вершина** через шов = T-3c (тело на колонке, не второй `face_key`); uid **не** от `location_uid` | два uid на одну прямую из‑за границы джобы или «uid для локации»; `face_key` = слот вершины | relief (C28 / T-3c) |
...
#### T-3c на шве чанков (locked)

**Зачем.** Каталог `face_key` клеит **одну прямую** (один Instance на ребре, rim-canonical, upsert `cell_refs`). Несколько сторон **одной вершины** — другой объект: `ReliefGradeSystem`. Юг месы в чанке A и восток в B связаны **телом плато на колонке шва**, не пересечением коридоров и не тем, что два фронта взяли один `face_key`.

Это уточнение «стыка после чанков» (C26 / C36 / R41), не новый шов и не второй пайплайн. Код — слой 6 ✅; этот пункт — SoT.

**Два стыка на одном C29 ребре (не смешивать)**

| Стык | Что | Когда | Не это |
|---|---|---|---|
| Каталог `face_key` | одна прямая `(kind, outward, θ)` → один Instance | uid до пула; paint по uid; после чанков — upsert `cell_refs` | две стороны одной месы; System row |
| T-3c на колонке шва | слоты, чьё **тело** 8-связно через ребро при той же целой z → одна вершина | после `compute_rect` **этого** refine | повторный discover; C41; catalog uid = вершина |

**Intra-chunk.** Два+ фронта одной вершины внутри одного `ColumnRect`: `at_grid` / `slot` уже одна вершина. Шов C29 **не** нужен. Post-pass группирует по slot, затем склеивает с матчами на рёбрах.

**Через два чанка.** На внутреннем `V|{cx}|{cy}` / `H|{cx}|{cy}` (оба чанка в этом вызове): у каждого воркера — клетки тела (`members` / `at_grid`) на **своей** стороне ребра. Матч: та же целая z и **8-сосед через шов** (угол — R41, не только орто). Не требовать, чтобы одна колонка входила в оба `ColumnRect`. Это **одно тело**. Union слотов и их Instance.

Противоположные фронты (юг в A, восток в B): если это одна меса, тело **пересекает** шов A|B. Матч — клетки тела **на колонке этого шва**, не overlap `cell_refs` коридоров. Если тела не касаются через ребро — это две вершины (два холма одной высоты), не System.
...
**Отпечаток воркера.** До post-pass воркер оставляет тонкий след на owned ребре: клетки тела (и кромки) на линии шва + какие Instance принадлежат слоту. На слоте бока Q2 — ещё parent слота (тот же bake-след, не SQL; `side_parent_slot`). Это bake-контракт (`ChunkComputeResult` / аналог), **не** колонка SQL, **не** поля persist-POJO (`occ` / slot / фронт — C38), **не** uid на клетке тела (R36t).

**Порядок.** После fill чанков (System не трогает z). После `merge_grade_instances` (C29 same-straight). `grade_instance_uids` упорядочены. Клетка → Instance, никогда System. 1 фронт → строки нет (`grade_system_uid` NULL), **кроме** бок-attach внутри rect (не этот шов): бок входит в System ближайшего тела — § R41 очереди. T-3c не подменяет бок-attach.
...
**Запрещено:** второй C39 / 8-flood макро-тайла / полный same-z CC; повтор `SeamStage` (C41); stamp uid на тело; расширять persist-POJO; второй пул; fill∥relief; ждать соседний `detailed_bake` ради System; считать catalog uid слотом вершины.

**Имена.** `SeamStage` / `ReliefVertices.seam` = шов **лучей** (C41). Post-chunk pass = стык на **техническом** шве (C29). Не называть T-3c «шов лучей».
...
| В компоненте | Entity |
|---|---|
| Одна прямая / один фронт | один `ReliefGradeInstance`; **без** строки System, **кроме** бок-attach Q2 (§ R41 очереди) |
| ≥2 прямых (смена kind / θ / outward; несколько сторон вершины) | N Instance + один `ReliefGradeSystem`; `grade_instance_uids` упорядочены. Стороны в разных чанках — слоты склеивает T-3c на C29, не catalog. Бок-attach — не этот ряд: тела разной z, intra-rect |
| 1 Grade | система **не** создаётся (R36l), **кроме** бок-attach Q2. Рабочий слот вершины на bake ≠ строка БД |
...
2. ~~**T-3c**~~ ✅ — System при ≥2 Instance **одной вершины**: intra-chunk по `slot`; через чанки — матч тел на C29 + UF по этому refine. Не «каталог увидел две прямые». Не слой 6 через макро-шов двух `detailed_bake`.
...
| Catalog uid на C29 | Слот вершины / System (T-3c — тело на колонке шва) |
| T-3c post-chunk на C29 | `SeamStage` / шов лучей (C41); второй C39 тайла |
...
| Документ | Связь |
|---|---|
| [`tz_terrain_generation.md`](./tz_terrain_generation.md) | skeleton / `N_eff`; pointer; **patch → тот же grade helper (R36v)**; SQL bulk эталон — TR-PERF-1 (**R43** = тот же приём на каталог грейдов, не `map_cells`) |
...
| [`tz_locations.md`](./tz_locations.md) | facing stairs; **barrier_template_registry**; локация на шве (**C29**); вершины layout — **R41** (не hunt пиков) |
...
| Дата | Изменение |
|---|---|
| 2026-08-26 | **`leftover_plus_halo`:** бывший `grade_ray_universe` — клетки leftover SLOPE/SHEER + 8-соседи в `z_height_map`. Не «вселенная». |
| 2026-08-26 | **R41-T-25 (критический P1):** алгоритм полного заполнения 8 слотов pack. Сначала полный fill; не игнор восьмёрки (dump `_`). Mill каскад без тела×8. Узкий «прямой склон = outward+`+`» — после алгоритма. T-17 ≠ occupancy. |
| 2026-08-25 | **R36t canal ≠ «старое»:** cut укороченного конца — когда земля упирается в structure / footprint (`L_eff` < requested, R36p). L=1 stamp — отдельное исключение (T-15). C24 оба. |
| 2026-08-25 | **Луч: occupied + known SHEER до SLOPE walk.** `FrontStage` не lockstep'ит занятый `(посадка, Facing)` и первый шаг с `slope_outcome(|dz|, 1) ≠ SLOPE` (пол конверта клетки). `propose_sheers` по-прежнему после ZQ (Q2). |
| 2026-08-25 | **R41-T-24 ✅:** relief `z_at` → `z_height_map` (метод, Mapping, Callable, consume). Не T-17. |
| 2026-08-25 | **`+` на same-z тела — не dump:** consume § Debug ASCII — таблица ответственности; T-17 пишет COUPLE, ASCII не invent. |
| 2026-08-25 | **Полиш mill T-20…T-23 ✅:** планировщик `millSchedule`; `DiscoverResult`; `BucketRef`/`MillFamily`/`Q2Kind`; `is_side_seed` = геометрия; leftover walk `iter_rect_z_cells`. Третий вид Q2 в SoT = бок коридора, не отдельный kind. |
| 2026-08-25 | **SoT mill в ТЗ:** вёдра Q1/Q2, цикл `z_top`, луч до упора, один walk — § Две очереди. C39 без второго скана rect. Registry **R41-T-18** / **T-19** — impl, не второй канон. |
| 2026-08-25 | **Open R36j:** ~~Q1/Q2/Q3~~ cell-fields → **R36j-A/B**, **R36m-A**. Не путать с mill Q1/Q2 (R41). |
| 2026-08-25 | **Q3 не очередь seed:** SoT = Q1/Q2 вёдра (**R41-T-18**). Снос mill `is_q3_seed` / `q3_s` / `q3_parent` — **R41-T-19**. Бок-attach persist остаётся (rename bake-полей в T-19). |
| 2026-08-25 | **Код vs SoT pack:** leftover-only sidecar ≠ 8 слотов. Чинить первым — **R41-T-17**. SoT записи — consume § Pack 8 слотов + R41/C41 станок (тело × 8 видов, same-z = `+`, следующие очереди не затирают). Не менять SoT pack. |
...
| 2026-08-18 | **T-3c слой 6 ✅:** `VertexSlotSeam` на `ChunkComputeResult`; `emit_relief_grade_systems` после `merge_grade_instances`; persist `systems`; 1 фронт → нет строки; клетка → Instance; UF этого refine. Макро-шов двух bake не impl |
| 2026-08-18 | **T-3c шов locked:** на C29 два стыка — catalog = прямая (Instance); T-3c = тело вершины (same-z 8 через колонку) + UF слотов этого refine. Intra-chunk = slot. Не C41, не второй C39, не catalog = вершина. Макро-шов двух `detailed_bake` — не слой 6. § T-3c на шве чанков |
...
| 2026-08-01 | **R36j:** Grade entity + bidirectional refs; cell=`system_grade_uid` only; h/L/angle on entity (**R36j-B** supersede) |
| 2026-08-01 | ~~R36j-A/B cell fields~~ (исторически Q1/Q2) superseded by R36j |
