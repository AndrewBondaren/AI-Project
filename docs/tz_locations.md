...
### Именованные объекты карты (`NamedLocation`)

**`NamedLocation` — опционально (U11).** Geometry не требует location.

**`NamedLocation`** — единая сущность «именованное место»: `display_name` + anchor (`map_x/y/z`) + `system_location_type`.

| Источник location | Когда | Кто |
|---|---|---|
| Master declare | import bundle | editor / import |
| **U13 materialize** | `materialize_named_locations: true` в **world template** | **`llm_name_procedural_locations`** (LLM) → python persist |
| Default autoresolve | geometry без declare | **нет** location |

Generators **не** пишут в `named_locations` и **не** вызывают LLM — только DAG ([`tz_world_generation_dag.md`](./tz_world_generation_dag.md)).
...
Общественная площадь (тег `plaza` в `structure_types`) и любое другое назначение шаблона — **NamedLocation** под районом. Площадь: `is_outdoor=true`, состав из шаблона. Не новый `location_type` **участка** (outdoor **C3**: area uid только в pack). В SQL-дереве parent = район (**C4**). Не тип ребра графа (connections §1).
...
[
  ...
  { "system_type": "geographic", "display_type": "География",  "parent_types": ["region","territory",null],  "is_outdoor": true,  "subtypes": [
      { "system_subtype": "mountain",     "border_category": null },
      { "system_subtype": "peak",         "border_category": null },
      { "system_subtype": "plain",        "border_category": null },
      { "system_subtype": "hill",         "border_category": null },
      { "system_subtype": "lake",         "border_category": "liquid" },
      { "system_subtype": "sea",          "border_category": "liquid" },
      { "system_subtype": "ocean",        "border_category": "liquid" },
      { "system_subtype": "inland_sea",   "border_category": "liquid" },
      { "system_subtype": "island",       "border_category": "liquid" },
      { "system_subtype": "coast",        "border_category": null },
      { "system_subtype": "river",        "border_category": null }
  ]},
  { "system_type": "climate_pole", "display_type": "Климатический полюс", "parent_types": [null], "is_outdoor": true, "subtypes": [] }
]
```
