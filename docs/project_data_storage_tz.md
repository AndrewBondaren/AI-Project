...
> **Продуктовое ТЗ модуля:** [`tz_world_snapshot.md`](./tz_world_snapshot.md) — единый `WorldSnapshotService`, capture **на каждый ход**, restore для time travel / regen / LOD.
...
> **Race contract (domain):** [`tz_races.md`](./tz_races.md) — prototype inheritance, gender blobs, merge, validation rules, fixture drift.  
> **Validator wire:** [`tz_json_validation.md`](./tz_json_validation.md) — SCH-RACE-ROW / SCH-RACE-CONTRACT.
...
Полный контракт L2/L3 blobs, appearance stack, ref graph — **[`tz_races.md`](./tz_races.md)**.
...
**Race contract** ссылается на схему и опционально ограничивает зоны — см. [`tz_races.md`](./tz_races.md) § body_schema.
...
> **TR-PACK (impl):** wilderness skeleton — **World Pack** (L0/L2); gameplay deltas — **`map_cell_patches`**. Legacy `map_cells` удалена. Read merge: field-wise overlay + `MapCellPatchLayerKind` — [`tz_world_pack_storage.md`](./tz_world_pack_storage.md) § WP-MERGE v2, § WP-FIX.

**Уровень 1 — `map_cell_patches`** — gameplay deltas поверх pack (бывш. `map_cells` wilderness):
```sql
map_cells (
  world_id,
  x, y,
...
**1. Раса** — врождённая способность; фолбек и агрегация — [`tz_races.md`](./tz_races.md) § terrain_access.
