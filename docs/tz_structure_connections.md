...
Порядок в районе (C21): packing xy участков → `_plan_streets` (z узла = terrain колонки, не пин района) → на каждый участок: `StructureAreaAssembler` (порог, опц. здание, `_build_paths`).

### 5.1.1 Подъезд к участку (`_build_paths`)

**Не делать:** копировать z улицы / пин района на `AreaSlot.ground_z` (outdoor **C21**).

Участок **не** равен зданию: двор + забор, здание у улицы, здание в глубине двора, площадка без дома. **Порог** (где улица стыкуется с участком) выбирает `StructureAreaAssembler._resolve_threshold` по топологии зоны — не формула «facing-ребро footprint здания».

| `kind` | Порог | Конец первого `yard_path` | Дальше |
|---|---|---|---|
| `door` | проём на улицу | `building_entrance` | `access_type` на двери |
| `gate` | ворота забора | `waypoint` на воротах | опц. `yard_path` по двору → дверь |
| `parcel_edge` | открытый край `slot.cells` | `waypoint` | опц. путь к двери |

«Дорога через двор к медиане bbox, пока дверь сидит на фасаде» — по-прежнему бессмысленна **для kind=door**. Для `gate` путь по двору как раз нужен: улица → ворота → (земля двора) → дверь.
...
|---|---|
| Вход **в дом** (`door`), крыльцо есть | `porch_material` |
| Вход **в дом**, крыльца нет | `parent_floor_material` |
...
`AreaLayout.connection_nodes` / `connection_edges` — выход `_build_paths`; extract пишет вместе с district graph. C20 (`front` дверь) — только если на участке есть здание.
