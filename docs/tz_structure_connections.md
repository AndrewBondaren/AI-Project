...
6. **Граф улиц** — главные по рамке, аллеи внутри модуля или кластера `center`, не через клетки участков.
...
| № | Источник | Когда |
|---|---|---|
| 1 | `required_structures[].count` | этот шаблон есть в `required_structures` района (`plot_template` = `system_name`) |
...
**Проход 1 — приоритетные (до рамки).** Состав: все `required_structures`, затем токены с `plot_priority` **> 0** (порядок — «Приоритет посадки»). Для **каждого** — бронь связного блока клеток решётки `block_size` во внутреннем bbox (спан `ceil(w/step)×ceil(d/step)`, 90°). Не через коридор якорей. Поставили — вынуть, не вытеснять. Нет места в bbox слота — `warning` / leftover; **не** отдавать это место обычной коллекции «пока ищем»; **не** расти `DistrictSlot`. Все токены с `position: center` сажаются **сначала**, как один кластер: первый в массиве (и копии `count`) — на домашний модуль inner bbox; следующие — жадно в 4-соседние модули (0 пустых модулей между ними). Рамка обходит **AABB hull** кластера (суперблок; улица рамки не между членами). Не встал в кластер — leftover + warning, не куда попало и **не** в проход 2. `plot_priority > 0` без `center` — first-fit после кластера, не в hull. Площадь в кластере — только пин чертежа с листом `plaza`; packing чертёж площади не подставляет. Канон civic — один `town_hall` с `center`. AABB брони — из cache; facing к полосе рамки — после шага рамки («Поворот оболочки»). Какой AABB до полосы — **CONN-PACK-3** (§8).
...
**Аллея.** Если в одном модуле ≥2 участка **или** в кластере `center` ≥2 участка и между ними нужен проезд — ребро `alley` из `connections` города/района и ширины §3.4. Нет аллеи в настройках — не выдумывать; либо щель packing без новой нитки. Аллея ≠ `PARCEL_GAP_M` и ≠ вторая главная улица. Грань участка 4-соседствует с главной сеткой **или** с этой внутренней аллеей.
...
**Footprint поселения:**  
`footprint_m = footprint_by_size[subtype][system_settlement_size] × world.fine_cells_per_map_cell`  
...
Settlement_gate-узлы ставятся на границах map_cell (координаты кратные `fine_cells_per_map_cell`).
...
| **CONN-PACK-2** — два (и более) `required_structures` с `position: center` | **closed:** один кластер вокруг home-модуля inner bbox района; рамка по AABB hull; не влезло — leftover, не проход 2. Канон civic — один `town_hall` |
