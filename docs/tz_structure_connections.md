...
**Целевой порядок модуля улицы и фасада (C22, §5.1.3–§5.1.4):** якоря/стены → **cache оболочек** → **проход 1 (бронь приоритетных)** → **рамка вокруг броней** → **проход 2 (остальная коллекция)** → **граф улиц** (рамка + внутренние аллеи, не сквозь участки) → слоту только касающиеся отрезки. Не «пустая сетка кварталов до зданий». Не «улицы целиком до packing» и не «packing по AABB района, потом overlay сетки». Код сейчас packing→overlay — расхождение. C21 по z не отменяется.
...
1. Якоря входа / стены района (нет в данных — скип). Внутренний bbox = район минус стена минус коридор `through_road` — контейнер packing. Коридор запрещён для участков (в т.ч. брони прохода 1).
...
| `walls` | стены района | поле `perimeter_barrier` есть → `reason=walls_always` + `template`; нет → `reason=skip_no_perimeter_barrier` |
...
| `frame` | рамка вокруг броней | `block_size`, число пустых кварталов, длина полос сетки (клетки), внутренний bbox после стен, число броней прохода 1 |
...
| `place` | сел | `uid`, origin xy, посаженные `w×h`, `facing`, грань к улице рамки |
...
| `front4` | tie равных рангов | счёт по ниткам, выбор или `reason=rng` + seed |

Якоря / стены без данных — одна строка `skip` с `reason`, не молчание. Cache без кандидатов — `warning`, не молчание.
...
| оба есть | стены + дырки под якоря; внутренний bbox = район минус стена минус коридоры; внутренность ≤ 0 → `warning`, не exception. Влезание `required_structures` — проход 1 **после** cache, не на этом шаге | — |
...
1. Якоря / стены (скип если нет в данных). Внутренний bbox не растёт.
...
6. Граф улиц по факту: главные по рамке, аллеи между слотами модуля, не через клетки участков. Нитка FRONT-4 (не для `plaza`).
7. `StructureAreaAssembler` — слот уже стоит; только касающиеся отрезки.
...
SettlementAssembler ставит узлы с шагом `block_size` — той же константой, что DistrictAssembler для решётки прохода 1 и рамки (`grid`). Узлы совпадают с решёткой без snap. Бронь прохода 1 не занимает клетки коридора; рамка не проводит главную улицу через бронь. Для `radial` / `organic` совпадение узел↔решётка — **CONN-PACK-1** (§8).

`block_size` по `settlement_density` (см. раздел 9, фаза 3): `dense=50 / medium=80 / sparse=120` **клеток**.
