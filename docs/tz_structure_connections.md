...
**Открытый вопрос:** `auto_sidewalk` у `highway` зависит от контекста — внутри города нужен, вне города нет. Механизм переопределения per-context не описан.
...
| Алгоритм прокладки `highway` между городами — A* с weighted cost function; референс: [Procedural Generation of Roads](https://www.researchgate.net/publication/229707505_Procedural_Generation_of_Roads), tmwhere | не описан |
| Алгоритм городской сетки улиц — референс: Parish & Müller (2001), tmwhere; паттерны: grid / radial / organic | не описан |
