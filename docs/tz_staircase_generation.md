...
```
march_depth     = depth − 1                (facing NS)   или   width − 1                (facing EW)
march_depth_mid = depth                    (facing NS)   или   width                    (facing EW)
```
`march_depth` — ступеней на **последний** марш.  
`march_depth_mid` — ступеней на **промежуточный** марш (нецелевые марши, все кроме последнего).

---

```
N₁ = width + 2·depth − 2        (facing север/юг)
N₁ = 2·width + depth − 2        (facing восток/запад)
N₂ = 2·width + 2·depth − 4      (multi-march, любой facing)
```
`N₁` — кол-во ячеек пути single-march U-обхода (3 ноги: из fr_anchor к far end и обратно).  
`N₂` — кол-во ячеек на один полный цикл (2 марша + 2 площадки) при multi-march.  
`width` — ширина interior шахты. `depth` — глубина interior шахты.

---

```
march_count = 1                                            (z_height ≤ N₁)
march_count = ceil(z_height / march_depth_mid)            (z_height > N₁)
  → если march_count нечётный и z_height − march_depth_mid ≥ march_count:
        march_count += 1   ← round up to even (BFS-инвариант)
```
`march_count` — кол-во прямых отрезков пути (маршей).  
Нечётный march_count создаёт ситуацию, когда два якоря на одной стене — BFS мог бы зациклиться. Поэтому принудительно округляем до чётного если это позволяет z_height.

---

```
flat_march1    = N₁ − z_height                            (march_count == 1)
march_cross    = width (facing NS)  или  depth (facing EW)
flat_per_march = march_cross − 2                          (march_count > 1)
```
`flat_march1` — кол-во stair_floor ячеек в single-march U-пути.  
`flat_per_march` — ширина площадки без угловых ячеек (угловые заняты якорями маршей).

---

```
march_steps[i] — ступеней на марш i; len == march_count
march_flat[i]  — target stair_floor на выходе марша i; последний марш = 0
Инвариант (промежуточные марши):
  march_steps[i] + march_flat[i] = march_depth_mid + 1
```
`march_steps` распределяет ступени по маршам: последний всегда `march_depth_mid`, остальные — остаток через divmod.  
Инвариант задаёт баланс «ступени + площадка» на каждом цикле.

---

```
L-марш (flat=2, width_int ≥ 3, не последний марш):
  inner_start = anchor + V                   ← staircase  на z
  corner_sf   = inner_start + V              ← stair_floor на z+1 (embedded в углу шахты)
  step2       = corner_sf + perp             ← staircase  на z+1
  perp = +turn_vector (i%2==0),  −turn_vector (i%2==1)
  → z += 2;  площадка-BFS ведёт к inner_start следующего марша
```
Оба угла торцевой стены (far/near) становятся stair_floor — «corners-first» раскладка.  
Валидатор логирует priority=CORNERS (цепочка stair_floor длиной 1).  
Стандартный марш (flat≠2) логирует priority=CENTER.

---

```
far_sz    = width (facing NS)  или  depth (facing EW)
lat       = depth − 2 (NS)     или  width − 2 (EW)
ideal     = far_sz
optimal   = far_sz + 2 × lat
mid_ideal = far_sz × 2
```
Пороговые значения для правил расстановки flat.  
`far_sz` = размер дальней стены. `lat` = длина боковой стены (без угловых ячеек).
...
для каждого марша i в range(march_count):
    pos    = anchors[i % 2]
    steps  = march_steps[i]          ← из массива (не обязательно march_depth_mid)
    flat   = march_flat[i]           ← 0 для последнего марша
    use_l  = (not is_last) and flat == 2 and width_int >= 3

    если use_l:
        # L-марш: inner_start → corner_sf → step2 (см. §0 L-марш)
        place L-march(pos, V, z, turn_vector, i)
        z += 2
    иначе:
    ...
    если i < march_count − 1:
        ...
        # Если следующий марш L-образный — BFS до inner_start(next), а не до anchor
        расставить stair_floor площадку (BFS по periметру interior)
...
`march_steps[i]` — ступеней на марш i (из `_compute_u_params`, последний = `march_depth_mid`).  
`march_flat[i]` — целевое кол-во stair_floor на выходе марша i.
...
| `ladder`         | Вертикальный переход (тип `trapdoor`). 1×1, один XY на всех z уровнях. Нет facing. |
...
| Условие | Формула |
|---|---|
| Single-march, N/S facing | `N₁ = width + 2·depth − 2` |
| Single-march, E/W facing | `N₁ = 2·width + depth − 2` |
| Multi-march (любой facing) | `N₂ = 2·width + 2·depth − 4` (перimeter, оба марша цикла) |

**Определение march_count:**
```
если z_height ≤ N₁  →  march_count = 1
иначе:
    march_count = ceil(z_height / march_depth_mid)
    если march_count нечётный и (z_height − march_depth_mid) ≥ march_count:
        march_count += 1    ← принудительно чётный

```
`march_depth_mid` = `depth` (NS) или `width` (EW) — ступеней на промежуточный марш.

Для 3×3 interior (NS): `N₁ = 7`, т.е. `z_height ≤ 7 → march_count = 1`.
...
```
# march_count = 1
flat_march1 = N₁ − z_height

# march_count > 1
march_cross    = width_interior (NS)  или  depth_interior (EW)
flat_per_march = march_cross − 2

# для каждого промежуточного марша (индекс i, не последний)
march_steps[i] + march_flat[i] = march_depth_mid + 1
```
`flat_per_march` фиксирован геометрически — это ширина площадки без угловых якорей.
...
### L-марш (flat=2, промежуточный марш)

Условие активации: `march_flat[i] == 2` и `width_int ≥ 3` и марш не последний.

Вместо `march_steps[i]` стандартных ступеней + BFS-площадки размещается L-образный путь:

```
anchor = (px, py)                            ← стартовый угол марша
inner_start = anchor + V                    ← staircase  на z     (шаг вперёд по V)
corner_sf   = inner_start + V              ← stair_floor на z+1  (угол шахты)
step2       = corner_sf + perp             ← staircase  на z+1   (поворот на perp)

V    = текущий вектор марша (mvx, mvy)
perp = +turn_vector (i % 2 == 0)  или  −turn_vector (i % 2 == 1)
```

Итог: 2 staircase-ступени, 1 embedded stair_floor → `z += 2`.

BFS лендинга ведёт к `inner_start` **следующего** марша (не к anchor), освобождая угол для stair_floor.

Оба угла торцевой стены становятся stair_floor — «corners-first» раскладка (priority=CORNERS).  
Стандартный марш (flat ≠ 2) → priority=CENTER.
...
## 8. Тип: `trapdoor` — лестница-люк

### Описание

Вертикальный переход поверхность ↔ подземный уровень. Занимает 1×1 ячейку, без шахты.  
Два варианта: **фиксированная** (встроенная в пол) и **передвижная** (TODO).

Управляется флагом `is_movable` в записи `staircases[]`:
```json
{
  "staircase_id": "cellar_ladder",
  "staircase_type": "trapdoor",
  "is_movable": false,
  "stops": ["cellar", "kitchen"]
}
```
`is_movable` по умолчанию `false`.

### Ячейки

| Тип ячейки | z уровни           | Описание                                       |
|------------|--------------------|------------------------------------------------|
| `ladder`   | z_lo .. z_top − 1  | Вертикальный переход, один XY на всех уровнях  |

`system_building_element = "ladder"` — не `"staircase"`, не `"trapdoor"`.  
Нет `system_facing` — движение строго вертикальное.

### Геометрический переход

**fr_anchor == to_anchor по XY** — оба конца лестницы находятся в одной точке плана.  
При шаге меняется только z, XY остаётся неизменным.

Якорь: SW-угол interior нижней комнаты (fr).

### Shaft

Trapdoor не создаёт shaft — `_NO_SHAFT_TYPES = {"trapdoor"}`.  
`clear_shaft()` и `lay_base_floor()` — no-op (shaft = None).

### Диспатч: fixed vs movable

```
TrapdoorBuilder.build()
  └── _compute_trapdoor_params(fr, to, sc_entry) → TrapdoorParams
        TrapdoorParams.is_movable == False → _build_fixed()
        TrapdoorParams.is_movable == True  → _build_movable()  ← TODO stub
```
...
```
trapdoor/
  __init__.py           → re-export TrapdoorBuilder
  trapdoor.py           → TrapdoorBuilder(StaircaseBuilder)
  trapdoorHelper.py     → TrapdoorParams, _compute_trapdoor_params, _compute_trapdoor_anchor
  trapdoorValidator.py  → TrapdoorValidator(StaircaseValidator) — TODO stub
```
...
class TrapdoorParams:
    anchor:     tuple[int, int]   # XY-позиция люка (fr_anchor == to_anchor)
    is_movable: bool
```

**`_build_fixed(params, z_lo, z_top, ...)`:**
- Размещает `ladder`-ячейки на `params.anchor` для каждого z в `[z_lo, z_top − 1]`
- `path_set = {(ax, ay, z) for z in range(z_lo, z_top)}`
- Возвращает `(fr_anchor, to_anchor)` — оба = `params.anchor`

**`_build_movable(params, ...)`:**
- `raise NotImplementedError` — TODO

### Открытые вопросы

- [ ] `_build_movable`: механика передвижной лестницы (предмет инвентаря?)
- [ ] Валидация: anchor должен быть в footprint нижней комнаты
- [ ] `_compute_trapdoor_anchor`: стратегия если SW-угол недоступен

---

## 8.1. Тип: `vertical_ladder` и `external_vertical_ladder`

### Принцип: нормализация направления

**Инвариант базового класса (StaircaseBuilder):**  
`fr` = нижняя комната (меньший z), `to` = верхняя комната (больший z).  
Нормализация выполняется в `StaircaseBuilder.__init__` — если `fr_level.z > to_level.z`, fr и to меняются местами.  
`z_lo = fr_level.z`, `z_top = to_level.z` всегда.  
Порядок `stops` в шаблоне не важен — генератор приводит к единому инварианту.

Это применимо ко всем типам лестниц (uShape, straight, spiral, vertical_ladder). Для uShape нормализация не влияет на построение пути (он управляется shaft-геометрией и z_lo/z_top).

### To-anchor-first подход

Якорь `to_anchor` определяется первым — как точка прихода в верхнюю комнату.  
Лестница строится от `z_lo` к `to_anchor`.

**`vertical_ladder` (внутренняя):**  
Якорь = пересечение interior нижней и верхней комнат (симметричная операция, нормализация не меняет результат).  
`fr_anchor == to_anchor` по XY — вертикальная шахта.

**`external_vertical_ladder` (внешняя):**  
Якорь = снаружи **верхней (to)** комнаты.  
`_anchor_outside(to, fr, ...)` — ищем позицию за пределами периметра to, предпочитаем кандидатов ближе к fr.  
`facing` задаёт сторону верхней комнаты, с которой выходит лестница.  
Проход прорубается в стене верхней комнаты (`_build_edge_ladder_passage`).  

### Параметры (поля `staircases[]`)

| Поле | Тип | Описание |
|---|---|---|
| `is_movable` | bool | Передвижная лестница (TODO) |
| `has_trapdoor` | bool | Физический люк на z_top |
| `has_walls` | bool | Стены шахты (3 стороны, кроме entry) |
| `facing` | str? | Сторона выхода наружу (north/south/east/west) |
| `open_wall_shaft` | str? | Материал окна в стенах шахты; None = глухие стены |
| `near_wall` | bool | Якорь допускает соседство со стенами (только internal) |
| `on_the_edge` | bool | Якорь за периметром fr (задаётся автоматически для external) |

### Нет shaft

Оба типа — в `_NO_SHAFT_TYPES`. Shaft не создаётся.  
`clear_shaft()` и `lay_base_floor()` — no-op.

### Ячейки

| Тип | z уровни | Описание |
|---|---|---|
| `ladder` | z_lo .. z_top − 1 | Вертикальная шахта, один XY |
| `trapdoor` | z_top | Физический люк (только если `has_trapdoor: true`) |

### Угловое ограничение (только external, is_movable=false)

Фиксированная внешняя лестница не может располагаться на углу верхней комнаты.  
Проверяется `ExternalVerticalLadderValidator._check_not_at_corner` по `to_footprint`.  
Угол = ячейка периметра с соседями-периметрами по обеим осям (X и Y).

### Подземный тоннель (external_vertical_ladder, z_lo < 0)

Если якорь снаружи верхней комнаты, на z_lo он может оказаться вне нижней комнаты (подвала).  
В этом случае строится подземный тоннель от якоря до стены нижней комнаты.

**Условие:** `z_lo < 0` И `anchor ∉ fr.get_footprint()`.

**Алгоритм:**
1. BFS с минимальным числом поворотов (0-1 BFS) от якоря до ближайшей ячейки `fr.get_footprint()`.
2. Путь делится на: `tunnel_cells = path[:-1]`, `entry = path[-1]` (первая ячейка fr footprint).
3. Каждая `tunnel_cell`:
   - `floor` на z_lo
   - `open` на z_lo+1 .. z_lo + z_height − 1
   - `wall` на всех z для боковых соседей не из пути и не из fr footprint
4. В `entry`: заменяем `wall` → `floor` (z_lo) + `open` (выше) — пролом в стену нижней комнаты.

**Ширина тоннеля:** 1 ячейка.  
**BFS:** 0-1 BFS — прямое движение (без поворота) = cost 0, поворот = cost 1. Минимизирует повороты.  
**Применимо:** только для `z_lo < 0` (подземный переход).

### Проход к верхней комнате (external_vertical_ladder)

После построения шахты `_build_edge_ladder_passage` прорубает стену **верхней (to)** комнаты:
- Находит ячейку периметра to, смежную с якорем
- Заменяет стену на `floor` (z_top) + `open` (z_top+1 .. z_top + z_height − 1)
...
| Basement / cellar             | `trapdoor` / `ladder`  |
