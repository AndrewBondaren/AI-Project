...
| Тема | Куда |
|---|---|
| Интерьеры (`StructureInteriorAssembler`: мебель, предметы) | [`tz_city_generation.md`](./tz_city_generation.md) §5 фаза 3; [`tz_assembler_hierarchy.md`](./tz_assembler_hierarchy.md) |
...
```
full_bake L0
  → orchestrator.plan_topology(world_uid)     # C23: районы + city gates; не packing
...
| Orchestrator | порядок: C23 topology после full L0; C11 packing; skip | алгоритм улиц/зданий |
...
Heartbeat — `packBakeLog` (тот же sink, что L2 `l2_s`). Не `materialize_s` (это fill колонок FineTerrain). Генератор `packBakeLog` не зовёт: assembler заполняет `SettlementAssembleTimings`, оркестратор пишет лог.

| Ключ | Этап |
|---|---|
| **`c11_s`** | Wall всего `materialize` после проверки pack |
...
| **`extract_s` / `encode_s` / `sql_s` / `publish_s`** | extract → zst tmp → SQL → publish+invalidate |
| **`topology_s` (C23)** | Wall одного `plan_topology`; `topo_terrain_s` / `topo_slots_s` / `topo_streets_s` / `topo_extract_s` / `topo_sql_s` |

HTTP `settlement.c11_pipeline`; скрипт `detailed_bake.py` дублирует в dump. Как читать vs L2 — [`tz_application_performance.md`](./tz_application_performance.md).
...
pack city structure
  ...
  districts[]          → uid + ref
    areas[]            → uid + AreaSlot + shell участка
      buildings[]      → named_location uid + клетки здания (поле объекта; wire `shell_cells`)
```
...
Клетки дома в pack на C11: полный `StructureLayout.cells` (полы, лестницы, внутренние стены, проёмы, фасад) и `small_layouts`. Фасад не отдельный persist-фильтр. L2 / WP-20 merge читают эти клетки. Наполнение (`StructureInteriorAssembler`) — не этот слой. `location_passages` между комнатами — C20 (не SQL в этом срезе).
...
| **C21** | **Онтология z участка.** Район **не** выравнивает участки: не одна плоскость, не `max(z)` AABB, не копия z улицы / `DistrictSlot.ground_z`. Участок — слот из **шаблона**, любое назначение (`structure_type`): не обязательно жилой дом (двор+забор, здание у улицы, в глубине, **площадь**, склад, храм, …). `building_location is None` — только если шаблон здания не даёт; шаблон даёт здание, участок пустой — **критическая ошибка generate** (assembler §7.1), не `plaza`. `ground_z` слота и **порог к улице** считает `StructureAreaAssembler` по топологии (`door` / `gate` / `parcel_edge` — connections §5.1.1). Не формула «всегда фасад здания». `building.map_z` (если дом есть) может отличаться от `AreaSlot.ground_z`. Луч порог→улица — **только если** `ground_z ≠ z` примыкающей улицы; иначе стык без профиля. Если луч даёт **θ > 45°** — assembler правит `map_z` **этого** дома (вверх или вниз), пока θ = 45° (§5.1.2). **Не** двигать здание в xy к улице. Соседние участки не трогать. Подъезд xy — `yard_path` к **порогу**. Pack-земля не терраса (C1). Пин района — не пол участка. |
| **C22** | **Онтология фасада** + посадка в **2D-бин модуля** (токены, корзины спан/один/остаток, жадный FFD; аллея из настроек если ≥2 в бине). Cache → бронь прохода 1 → рамка вокруг → проход 2; граф после. Слоту только касающиеся отрезки. Нитка `frontage`; `plaza` — улицы равны. **`PerimeterBarrier`:** один класс, три инстанса, не синглтон. Барьер **поселения** — прямые footprint (`SettlementLayout.barrier_cells`); `SettlementAssembler` вычитает `footprint ∩ слот` из площади района до packing. Барьер **района** — прямые урезанного слота; packing вычитает их. Участок — `_build_barrier`. Зоны не пересекаются. Стены здания — не этот класс. Число копий — §5.1.3 «Число токенов». Очередь — §5.1.3 «Приоритет посадки». Оболочку packer **может** 90°; интерьер не rotate; вход участка = парадный основного дома — §5.1.3 «Поворот оболочки». Debug — §5.1.3 «Debug packing». Переход кода — **TODO** city §6.3. SoT: connections **§5.1.3–§5.1.4**, city **§6.3**. |
...
| **C21-T7** | Участок ≠ только дом | любой `structure_type` из шаблона; C20 только если здание с входом; шаблон даёт здание + пустой участок = **ошибка generate** (assembler §7.1) | packing сажает шаблоны cache из `allowed_structure_types`; нет fail, если шаблон с домом собран без NL | Stamp малых объектов площади — assembler §7.8; hard-fail сломанного участка |
...
| 2026-09-13 | **C11/C23 timings:** `packBakeLog` start/done + `c11_pipeline` / `topology_pipeline`. Не путать с L2 `materialize_s`. |
| 2026-09-13 | **C8:** pack несёт полную геометрию дома (`StructureLayout.cells`); фасад не persist-фильтр. Мебель — InteriorAssembler. `location_passages` — C20. |
...
| 2026-09-01 | C22: cache → бронь приоритетных → рамка вокруг → проход 2. `DistrictSlot` packing не растит. SoT «Пайплайн посадки». |
| 2026-09-01 | C22: проход 1 = приоритетные (место под каждое); проход 2 = остальная коллекция. Не «первый токен». SoT «Два прохода посадки». |
| 2026-09-01 | C22: packer может 90° оболочку; вход участка = парадный основного дома; интерьер не rotate. SoT connections §5.1.3 «Поворот оболочки». |
