...
| **C22** | **Онтология фасада** + посадка в **2D-бин модуля** (токены, корзины спан/один/остаток, жадный FFD; аллея из настроек если ≥2 в бине). Cache → бронь прохода 1 → рамка вокруг → проход 2; граф после. Слоту только касающиеся отрезки. Нитка FRONT-4; `plaza` — улицы равны. Стены: поле есть → всегда. Число копий — §5.1.3 «Число токенов». Очередь — §5.1.3 «Приоритет посадки». Оболочку packer **может** 90°; интерьер не rotate; вход участка = парадный основного дома — §5.1.3 «Поворот оболочки». Debug каждого шага — §5.1.3 «Debug packing». Переход кода — **TODO** city §6.3. SoT: connections **§5.1.3–§5.1.4**, city **§6.3**. |
