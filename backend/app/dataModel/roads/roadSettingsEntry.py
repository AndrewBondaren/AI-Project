...
class RoadSettingsEntry(BaseModel):
    ...
    curve_radius_factor: OptionalOnWire[int] = constrained_field(default=1, greater_equals=1)
    max_segment_length_m: OptionalOnWire[int] = constrained_field(default=30, greater_equals=1)
    min_segment_length_m: OptionalOnWire[int] = constrained_field(default=3, greater_equals=1)
    default_lanes_per_side: OptionalOnWire[int | None] = None
    auto_sidewalk: OptionalOnWire[bool] = False
    base_travel_modifier: OptionalOnWire[float] = constrained_field(default=1.0, greater=0.0)
    condition_degradation: OptionalOnWire[float] = constrained_field(default=0.2, greater_equals=0.0)
