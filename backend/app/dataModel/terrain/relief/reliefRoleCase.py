"""One slope policy case — Mode A (delta_z) or Mode B (bands) — tz_terrain_relief R32."""
...
def _reject_band_overlap(bands: list[ReliefDeltaBand]) -> None:
    ...
    ordered = sorted(bands, key=lambda b: (b.delta_z_min, b.delta_z_max is None, b.delta_z_max or 0))
    prev_max: int | None = None
    for band in ordered:
        if prev_max is not None and band.delta_z_min <= prev_max:
            raise ValueError(
                f"overlapping bands: delta_z_min={band.delta_z_min} overlaps previous max={prev_max}"
            )
        if band.delta_z_max is None:
            prev_max = UNBOUNDED_DELTA_Z_MAX  # blocks further bands after unbounded
        else:
