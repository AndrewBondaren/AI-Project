...
from app.dataModel.connections.connectionType import connectionTypeEntry as _entry_mod
...
class WorldConnectionTypeRegistry(RootModel[list[ConnectionTypeEntry]]):
    ...
    def keys(self) -> frozenset[ConnectionTypeKey]:
        return frozenset(e.system_connection_type for e in self.root)

    def require(self, system_connection_type: str) -> ConnectionTypeKey:
    ...
    @classmethod
    def require_engine(cls, system_connection_type: str) -> ConnectionTypeKey:
        return cls.canonical_engine().require(system_connection_type)

    @classmethod
    def road_mask_connection_types(cls) -> tuple[ConnectionTypeKey, ...]:
        return tuple(cls.require_engine(k) for k in ROAD_MASK_CONNECTION_TYPE_KEYS)

    @classmethod
    def hydrology_connection_types(cls) -> tuple[ConnectionTypeKey, ...]:
...
_entry_mod.WorldConnectionTypeRegistry = WorldConnectionTypeRegistry
ConnectionTypeEntry.model_rebuild()
