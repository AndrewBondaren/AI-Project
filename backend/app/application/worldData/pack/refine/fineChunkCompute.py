"""One ColumnRect worker: grade sample+plan+materialize then column fill."""
...
def compute_rect(
    ...
    """Grade stays inside the ColumnRect task (R36v / R36w). No persist here."""
    chunk_idx, rect = pair
    ...
    if ctx.templates:
        batches = sample_detailed_grade_rect(
            ctx.world, ctx.surface_state, rect,
            halo=ctx.grade_halo, existing_uids=ctx.existing_uids,
            job_uid=ctx.catalog.job_uid_chunk(
                *ctx.catalog.chunk_of_rect(rect),
            ),
        )
        planned = plan_rect_grade(
            ctx.world, batches, ctx.catalog,
            relief_templates_by_uid=ctx.templates,
            surface_state=ctx.surface_state,
        )
        if planned:
            part = materialize_planned_for_rect(
                ctx.world, ctx.surface_state, rect, planned,
                existing_uids=ctx.existing_uids,
                catalog=ctx.catalog,
            )
            chunk_grades = part.grade_instances
            chunk_state = replace(
                ctx.surface_state, surface_grade_uid=part.surface_grade_uid,
            )
