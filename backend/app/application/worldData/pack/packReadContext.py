...
class PackReadContext:
    ...
    def reader_for(self, world: World) -> WorldPackReader:
        ...
        if reader is None:
            reader = WorldPackReader(paths, fine_terrain_cache=self._fine_terrain_cache)
            self._readers[key] = reader
        return reader

    @property
    def fine_terrain_decode_cache(self) -> FineTerrainDecodeCache:
        return self._fine_terrain_cache
